...
| ocp_meta_username  |  可选    | meta 租户下的用户名。<blockquote> **说明**：</br> 这里的用户名格式为 useraname@tenant_name，如果只写 username 默认使用 username@sys 连接。</blockquote>            |
...
3. 修改配置文件
   ...
   | jdbc_username  |  必选    | 连接 meta 租户的用户名，请确保该用户已创建。<blockquote> **说明**：</br> 这里的用户名格式为 useraname@tenant_name，如果只写 username 默认使用 username@sys 连接。</blockquote>                |

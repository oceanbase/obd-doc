...
* 内存：需为 OCP Express 预留 512 MB 以上内存，长稳运行建议 762 MB 以上。
...
|  配置项            | 是否必选  |         描述                         |
|--------------------|----------|--------------------------------------|
| tenant_name        |  可选    | 为 OCP Express 创建的 meta 租户名称。  |
| max_cpu            |  可选    | 为 meta 租户分配的最大 CPU。           |
| memory_size        |  可选    | 为 meta 租户分配的内存容量。            |
| ocp_meta_db        |  可选    | OCP Express 中数据存储所需的数据库。    |
| ocp_meta_username  |  可选    | meta 租户下的用户名。                |
| ocp_meta_password  |  可选    | 用户密码。                             |

<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>上述配置项都不配置的情况下，OBD 会按照默认规格为 OCP Express 创建 meta 租户。默认规格如下：</p>
  <ul>
  <li>
...
2. 为 OCP Express 创建一个 meta 租户，并创建执行操作所使用的用户，具体操作步骤可参考 [创建用户租户](https://www.oceanbase.com/docs/community-observer-cn-0000000000965467)。
...
3. 修改配置文件
   ...
   | jdbc_username  |  必选    | 连接 meta 租户的用户名，请确保该用户已创建。                |
   ...
   | address  |  必选    | OBServer 节点的 IP 地址。                             |
   | svrPort        |  必选   | OBServer 节点的 rpc 端口，需和  oceanbase-ce 组件中对应节点的 `rpc_port` 配置项相同。  |
   | sqlPort            |  必选    | OBServer 节点的 mysql 端口，需和 oceanbase-ce 组件中对应节点的 `mysql_port` 配置项相同。           |
   | agentMgrPort        |  必选    | OBAgent 管理进程的端口。            |
   | agentMonPort        |  必选    | OBAgent 监控进程的端口。    |

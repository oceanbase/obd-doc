...
解决方法：可以执行 `obd cluster component add <deploy_name> -c <ob-configserver.config>` 命令新增 obconfigserver 组件，具体操作可参考 [扩容与组件变更](400.user-guide/300.command-line-operations/300.manage-cluster/900.expansion-and-component-changes.md) 中 **组件新增** 部分。
...
解决方法：升级前请关闭 HA 功能，您可登录 OMS 社区版控制台，在左侧导航栏中单击 系统管理 > 系统参数。单击 `ha.config` 取值列后的编辑图标，在 **修改取值** 对话框中，设置 `enable` 为 `false`，禁用 HA 功能。

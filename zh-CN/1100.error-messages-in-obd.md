...
<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>在 <code>redo_dir</code> 和 <code>data_dir</code> 同盘的情况下，计算磁盘使用率时会算上 datafile 将要占用的空间。</p>
</main>
...
错误原因：OBServer 节点的操作系统不支持 avx 指令集。

解决方法：请更换所用机器，使用支持 avx 指令集的机器部署 OceanBase 数据库。
...
### OBD-2016：Primary tenant {primary_tenant} have not full log, not support create standby cluster

错误原因：基于网络创建备租户时要求主租户需拥有完整的日志信息，可登录主租户的 root 用户执行 `SELECT LS_ID, BEGIN_LSN FROM oceanbase.GV$OB_LOG_STAT WHERE ROLE = 'LEADER' ;` 命令查看，输出中 `BEGIN_LSN` 值为 `0` 表示当前日志流副本拥有自创建以来的完整日志。

解决方法：若当前主租户未拥有自创建以来的完整日志，可为主租户执行归档，并基于归档创建备租户，具体步骤可参见 [创建备租户](400.user-guide/300.command-line-operations/300.manage-cluster/800.physical-standby-database/100.deploy-physical-standby-database.md) 中 **基于归档创建备租户**。创建完成后，若仍旧希望基于网络同步日志，可参照 [切换主备租户同步方式](400.user-guide/300.command-line-operations/300.manage-cluster/800.physical-standby-database/230.switch-log-source.md) 一文，将主备租户的同步方式修改为基于网络同步。

### OBD-2017：Continuous log synchronization is not enabled

错误原因：基于归档创建备租户后，需为备租户启用日志同步，以保证主备租户间的日志同步。

解决方法：可执行 `obd cluster tenant recover` 命令为备租户启动日志同步，命令介绍可参见 [集群命令组](300.obd-command/100.cluster-command-groups.md)，搜索 **obd cluster tenant recover** 即可查看。
...
错误原因：级联主备的场景下，使用基于归档的方式为主租户（A_a）创建备租户（B_a）后，不支持使用基于网络的方式为该备租户（B_a）创建备租户（C_a）。

解决方法：若希望为基于归档创建的备租户（B_a）继续创建备租户，可选择基于归档的方式创建备租户，具体操作可参见 [创建备租户](400.user-guide/300.command-line-operations/300.manage-cluster/800.physical-standby-database/100.deploy-physical-standby-database.md) 中 **基于归档创建备租户**。

### OBD-2020：Multiple observer nodes on the same server are not supported by the auto start feature

错误原因：同一个服务器上启动多个 observer 进程时开启了 observer 进程自启动功能（`enable_auto_start` 为 `true`）。

解决方法：有如下两种解决方法，请根据实际情况选择。

- 方法一：执行 `obd cluster edit-config <deploy name>` 命令打开配置文件，修改 `enable_auto_start` 为 `false` 关闭 observer 进程自启动功能。

- 方法二：控制一个服务器上只启动一个 observer 进程。

### OBD-2021：Permission denied. Current user {user} on server {ip} requires sudo privileges

错误原因：配置的用户未拥有 sudo 权限。

解决方法：有如下两种解决方法，请根据实际情况选择。

- 方法一：为对应节点机器中的用户配置 sudo 权限。

- 方法二：单击 **上一步**，在 **节点配置** 页面更新用户配置，使用拥有 sudo 权限的用户。

  <main id="notice" type='explain'>
    <h4>说明</h4>
    <p>填写用户时需确保用户在每个节点中都存在，且都拥有 sudo 权限。</p>
  </main>
...
错误原因：当前业务集群中未部署 obconfigserver。

解决方法：有如下两种解决方法。

- 方法一：为 `obd binlog create` 命令配置 `--rs` 选项，使用集群的 OBServer 节点列表（`SHOW PARAMETERS LIKE 'rootservice_list';` 命令查看 `value` 值）为指定的租户创建 Binlog 实例。
  
  <main id="notice" type='explain'>
    <h4>说明</h4>
    <p><code>obd binlog create</code> 命令的 <code>--rs</code> 选项自 obd V4.2.0 新增，且仅在 obbinlog 为 V4.3.5 或之后版本时支持配置。</p>
  </main>

- 方法二：可以执行 `obd cluster component add <deploy_name> -c <ob-configserver.config>` 命令新增 obconfigserver 组件，具体操作可参考 [扩容与组件变更](400.user-guide/300.command-line-operations/300.manage-cluster/900.expansion-and-component-changes.md) 中 **组件新增** 部分。
...
错误原因：社区版 Binlog 服务只能用于 OceanBase 数据库社区版。

解决方法：如需对 OceanBase 数据库企业版使用 Binlog 服务，请部署企业版 Binlog 服务。部署操作可参见 [部署 obbinlog](400.user-guide/300.command-line-operations/200.deploy-component/450.deploy-binlog.md)。
...
错误原因：企业版 Binlog 服务只能用于 OceanBase 数据库企业版。

解决方法：如需对 OceanBase 数据库社区版使用 Binlog 服务，请部署社区版 Binlog 服务。部署操作可参见 [部署 obbinlog](400.user-guide/300.command-line-operations/200.deploy-component/450.deploy-binlog.md)。

## OMS 相关报错

### OBD-4701：failed to connect meta db

错误原因：在使用外部数据库作为 MetaDB 部署 OMS 时，无法连接 MetaDB。

解决方法：请执行 `obd cluster edit-config <deploy_name>` 命令打开并排查配置文件中 MetaDB 配置是否正确。若内容有误，请根据正确信息更新。

### OBD-4702：failed to connect influxdb

错误原因：根据配置的时序数据库的访问信息无法连接到对应时序数据库。

解决方法：请执行 `obd cluster edit-config <deploy_name>` 命令打开并排查配置文件中时序数据库部分配置是否正确。若内容有误，请根据正确信息更新。

### OBD-4703：({ip}) {disk} not enough disk space

错误原因：选择**在线升级**方式升级 OMS 时，需要指定存放升级文件的路径，指定的路径磁盘空间不足。

解决方法：请配置为磁盘空间充足的目录。

### OBD-4704：HA is enabled, please disable it before upgrade

错误原因：升级前需禁用高可用模块（High Available，简称 HA）。HA 为数据传输链路的稳定性和连续性提供保障，具体介绍可参见官网《OceanBase 迁移服务》文档 [OMS 社区版简介/OMS 社区版高可用（HA）介绍](https://www.oceanbase.com/docs/community-oms-cn-1000000004253137)。

解决方法：升级前请关闭 HA 功能，您可参考如下操作关闭：

...
错误原因：`proxy_id` 配置项的取值不在有效范围内。根据 `client_session_id_verison` 配置项的取值不同，`proxy_id` 配置项的有效范围有如下两种情况。

- 配置项 `client_session_id_verison` 的值配置为 `1` 时，配置项 `proxy_id` 的有效范围为 \[1,255]。

- 配置项 `client_session_id_verison` 的值配置为 `2` 时，配置项 `proxy_id` 的有效范围为 \[1,8191]。

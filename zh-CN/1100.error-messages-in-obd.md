...
- OCP Express 控制台的 admin 登录密码（`admin_passwd`）需满足如下规范：
  ...
  <main id="notice" type='notice'>
    <h4>注意</h4>
    <p>OCP Express 中 admin 登录密码（<code>admin_passwd</code>）修改后需执行 <code>obd cluster redeploy</code> 命令重启生效。<code>obd cluster redeploy</code> 命令会销毁集群，重新部署，您集群中的数据会丢失，请先做好备份。</p>
  </main>

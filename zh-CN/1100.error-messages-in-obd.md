...
解决办法：您可执行 `obd cluster edit-config` 命令打开对应配置文件，并修改 `admin_passwd` 配置项，该配置项复杂度需满足：长度为 8~32 位字符，包含以下四种类型字符中至少三种：数字（0\~9）、大写字母（A\~Z）、小写字母（a\~z）和特殊字符（<code>~!@#%^&*_-+=|(){}[]:;,.?/$`'"<>\\</code>）。
...
### OBD-4366：'(xx.xx.xx.xx) ocp admin_password invalid

错误原因：配置的 OCP 管控台登录密码（`admin_password`）不满足密码要求。

解决方法：您可执行 `obd cluster edit-config` 命令打开配置文件，修改 `admin_password` 配置项，使配置的密码满足如下密码要求。

- 密码长度为 8~32 位

- 包含以下四种类型字符中至少三种：数字（0\~9）、大写字母（A\~Z）、小写字母（a\~z）、特殊符号（<code>~!@#%^&*_-+=|(){}[]:;,.?/$`'"<>\\</code>）

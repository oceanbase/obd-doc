...
错误原因：对应组件配置的密码不符合规范。OBD 会根据组件的密码规范对组件进行检查，OCP 控制台的 admin 登录密码需满足如下规范。

- 长度为 8~32 位字符

- 包含以下四种类型字符中至少三种：数字（0\~9）、大写字母（A\~Z）、小写字母（a\~z）和特殊字符（<code>~!@#%^&*_-+=\|(){}[]:;,.?/$`'"<>\\</code>）

解决方法：根据部署的方法不同，有如下两种解决方法。

- 白屏部署
  
  白屏部署的错误码出现在 预检查 页面，您可单击 上一步，找到对应组件的对应配置，修改密码使其符合规范

- 命令行部署
  
  您可执行 `obd cluster edit-config` 命令打开配置文件，修改配置文件中对应组件的对应配置项并保存，之后复制命令行输出的重启命令并执行。
...
解决办法：您可执行 `obd cluster edit-config` 命令打开对应配置文件，并修改 `admin_passwd` 配置项，该配置项复杂度需满足如下条件。

- 长度为 8~32 位字符。

- 支持字母、数字和特殊字符，且至少包含大、小写字母、数字和特殊字符各 2 位。

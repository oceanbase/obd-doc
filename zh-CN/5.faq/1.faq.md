...
1. 单击下载官方资源包 [OpenJDK 1.8.0](https://download.java.net/openjdk/jdk8u42/ri/openjdk-8u42-b03-linux-x64-14_jul_2022.tar.gz)，并将其上传到对应机器。

2. 在资源包所在目录下执行如下命令解压安装。

   ```bash
   # 解压资源包
   tar -zxvf openjdk*.tar.gz -C openjdk8
   # 进入解压后的目录
   cd openjdk8
   # 创建软连
...
1. 下载 [OpenJDK Alibaba](https://github.com/alibaba/dragonwell8/releases/download/dragonwell-extended-8.14.15_jdk8u362-ga/Alibaba_Dragonwell_Extended_8.14.15_aarch64_linux.tar.gz) 开源资源包，并将其上传到对应机器。
2. 在资源包所在目录下执行如下命令解压安装

   ```bash
   # 解压资源包
   tar -zxvf Alibaba_Dragonwell_Extended_8*.tar.gz -C openjdk8
   # 进入解压后的目录
   cd openjdk8
   # 创建软连
   ln -s `pwd`/bin/java /usr/bin/java

...
由于开源 OBProxy 组件正式更名为obproxy-ce，因此使用以下命令升级会报 `No such package obproxy-3.2.3`。

```shell
obd cluster upgrade <deploy name> -c obproxy -V 3.2.3
```

您需在 OBD 的执行用户下执行下述 [脚本](./2.how-to-upgrade-obproxy-to-obproxy-ce-3.2.3.md/#执行脚本) 修改 meta 信息，而后使用以下命令对 OBProxy 进行升级。

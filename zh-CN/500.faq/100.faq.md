...
## 安装 all-in-one 后安装 OBD，无法正常使用怎么办？

若您在已经安装了 all-in-one 安装包的情况下，通过 obd RPM 包安装 obd 时出现如下提示：
...
<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>不建议在安装 all-in-one 后再单独安装 obd RPM 包，如需使用其他版本的 obd，建议安装对应版本的 all-in-one。安装 all-in-one 的操作可参见 <a href="../200.quick-start/100.install-obd.md">安装并配置 obd</a> 中 <b>使用 all-in-one 安装包安装 obd</b>。</p>
</main>

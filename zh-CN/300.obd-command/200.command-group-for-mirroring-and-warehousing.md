...
使用该命令可清理已下载到机器（`~/.obd/mirror`）和已通过 obd 安装（`~/.obd/repository`）的指定文件。
...
<main id="notice" type='explain'>
  ...
  <ul>
  <li>
  <p>未指定选项的情况下，obd 会清理所有当前未被使用的组件文件，并确保每个组件会保留一个最新版本的 RPM 包，是否为最新 RPM 包由版本号的大小排序判定。</p>
  </li>
  <li>
  <p>除 <code>--hash</code> 选项外，每个选项配置后都将清理除最新版本和当前使用版本外所有符合选项条件的文件。</p>
  </li>
  </ul>
</main>
...
| 选项名             | 是否必选 | 数据类型   |  说明                        |
|-----------------|------|--------|---------------------------|
| -c/--components | 否      | string | 指定组件名，将清理 `~/.obd/mirror` 和 `~/.obd/repository` 目录下该组件除最新版本和当前使用版本外的所有包，是否为最新 RPM 包由版本号的大小确定。配置为多个值时使用英文逗号（`,`）分隔。     |
| -t/--type     | 否     | string   | 可选值为 `rpm` 和 `repository`，指定为 `rpm` 时仅清理 `~/.obd/mirror` 目录下的文件；指定为 `repository` 时仅清理 `~/.obd/repository` 目录下的文件。   |
| --hash        | 否     | string   | 指定安装包的 hash 值，将清理 `~/.obd/mirror` 和 `~/.obd/repository` 目录下对应 hash 值的文件，配置为多个值时使用英文逗号（`,`）分隔。<main id="notice" type='explain'><h4>说明</h4><p>可通过配置 <code>--hash</code> 选项指定删除组件的最新版本文件。</p></main>         |
| -y/--confirm  | 否     | bool    | 用于控制是否进行二次确认。  |

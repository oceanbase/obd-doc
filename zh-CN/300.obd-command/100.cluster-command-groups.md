...
```shell
obd cluster tenant backup <deploy name> <tenant name> <server ip> <data backup uri> <archive log uri> [options]
...
* `tenant name` 为待备份租户的租户名，指定时需确保租户已存在。

* `server ip` 可设置为 OceanBase 数据库中任一节点的 IP。

* `data backup uri` 用于设置存储数据备份文件的目录路径。

* `archive log uri` 用于设置存储日志归档文件的目录路径。
...
<!-- | -P/--plus_archive   | 否  | bool  |     | 控制是否在数据和日志组合备份的备份过程中包含归档日志。 |--确认是否需要传值，而且我没有设置的时候就自动备份了日志呀 -->
...
<!-- 我们现在是仅支持到了租户级恢复对不，支持表级恢复么 -->
使用该命令可将指定路径下的备份内容恢复到指定租户，目前仅支持租户级恢复。

```shell
obd cluster tenant restore <deploy name> <tenant name> <server ip> <data backup uri>  <archive log uri> <zone> <memory_size> <max_cpu> [options]

#example
obd cluster tenant restore test obmysql 10.10.10.1 'file:///data/nfs/backup/data' 'file:///data/nfs/backup/log' zone1 2G 2
```
...
* `zone` 用于设置待恢复租户所属的 Zone 名称。

* `memory_size` 用于设置待恢复租户的可用内存，建议尽量与源租户保持一致。

* `max_cpu` 用于设置待恢复租户的最大可用 CPU，最小值为 1，建议尽量与源租户保持一致。
...
| -D/--decryption           | 否   | []string  |        | 备份集设置的密码，如设置了多个密码需写出全部的密码。                                |
...
使用该命令可取消租户备份。
<!-- 如果我将一个租户的数据分多次备份到了多个目录下，那我执行取消命令取消的是那一次的呢，执行查看命令查看的又是哪一次的呢 -->
```shell
obd cluster tenant backup-cancel <deploy name> <tenant name> <server ip>
...
```shell
obd cluster tenant restore-cancel <deploy name> <tenant name> <server ip>

#example
obd cluster tenant restore-cancel test obmysql 10.10.10.1
```

参数 `deploy name` 为部署名，可以理解为配置文件的别名；`tenant name` 为待恢复租户的租户名，指定时需确保租户已存在。
...
```shell
obd cluster tenant backup-show <deploy_name> <tenant_name> <server ip>
...
```shell
obd cluster tenant restore-show <deploy name> <tenant name> <server ip>

#example
obd cluster tenant restore-show test obmysql 10.10.10.1
```

参数 `deploy name` 为部署名，可以理解为配置文件的别名；`tenant name` 为恢复租户的租户名，指定时需确保租户已存在。
...
使用该命令可以接管非 obd 部署的 OceanBase 数据库。具体操作方法可参见 [使用 obd 接管集群](../400.user-guide/300.command-line-operations/450.takeover-cluster.md)。
...
使用该命令可通过传入 yaml 格式配置信息，对正在运行的集群中指定组件进行扩容。扩容的详细操作可参见 [扩容与组件变更](../400.user-guide/300.command-line-operations/900.expansion-and-component-changes.md) 中 **集群扩容**。
...
使用该命令可通过传入 yaml 格式配置信息，向正在运行的集群中添加新的组件。新增组件的详细操作可参见 [扩容与组件变更](../400.user-guide/300.command-line-operations/900.expansion-and-component-changes.md) 中 **组件新增**。
...
使用该命令可删除集群中的指定组件（可多选）。使用该命令时要求集群需处于运行状态。删除组件的详细操作可参见 [扩容与组件变更](../400.user-guide/300.command-line-operations/900.expansion-and-component-changes.md) 中 **组件删除**。

...
|-----------------------|------|--------|--------------------------|------------------------------------------------------------------------|
| --type  | 否  | string   | SERVICE  | 指定备租户同步数据方式，可配置为 `LOCATION`（基于归档）或 `SERVICE`（基于网络）。 |
| -a/--archive_log_uri  |  否   | string   | 主租户备份时设置的日志归档文件的目录路径   | 设置存储日志归档文件的目录路径，备租户会从该路径中获取主租户的日志归档信息。当 `--type` 配置为 `LOCATION`，且未配置该选项时，obd 会自动获取主租户备份（`obd cluster tenant set-backup-config` 命令）时设置的日志归档文件路径。<main id="notice" type='explain'><h4>说明</h4><p>当备租户访问不到主租户配置的备份信息时需要指定该选项，如：主备租户跨机房时，备租户无法访问主租户的 NFS，此时可以将日志归档文件复制到备租户可以访问的路径，并配置该选项。</p></main>      |
| --access_id  | 否  | int  | 无  | 当使用 OSS 或 S3 作为备份介质，且未配置 `-a`，或 `-a` 中未包含 access_key 信息时，需通过该选项指定 access_id。 |
| --access_key | 否  | string  | 无  | 当使用 OSS 或 S3 作为备份介质，且未配置 `-a`，或 `-a` 中未包含 access_key 信息时，需通过该选项指定 access_key。 |
| --s3_region  | 否  | string  | 无 | 当使用 S3 作为备份介质时，通过该选项指定 S3 存储桶所在的地理区域。 |
...
| --unlimited | 否  | bool | 无 | 设置是否连续回放存档的源日志，无需传值，命令中出现该选项即表示设置连续回放存档的源日志。 |
...
| --standbyro-password | 否 | string  | 随机字符串  | 主租户提供给备租户的用户名（`standbyro`）对应的密码，不允许为空。若之前已经创建过 standbyro 用户（非 create-standby 命令创建），该选项为必选项。 <main id="notice" type='explain'><h4>说明</h4><p>standbyro 用户密码中不允许包含：英文单引号（<code>'</code>）、英文双引号（<code>"</code>）、反引号（<code>`</code>）、英文分号（<code>;</code>）及空格（<code> </code>）。</p></main>   |
| --access_id  | 否  | int  | 无  | 当使用 OSS 或 S3 作为备份介质时，通过该选项指定 access_id。 |
| --access_key | 否  | string  | 无  | 当使用 OSS 或 S3 作为备份介质时，通过该选项指定 access_key。 |
| --s3_region  | 否  | string  | 无 | 当使用 S3 作为备份介质时，通过该选项指定 S3 存储桶所在的地理区域。 |

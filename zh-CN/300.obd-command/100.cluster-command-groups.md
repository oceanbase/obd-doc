...
使用该命令可在主租户可用的情况下，将主租户与其中一个备租户交换角色。
...
选项 `-p` 为 `--tenant-root-password`，配置为备租户 root 用户的登录密码。备租户 root 用户的登录密码默认与主租户保持一致，且不支持修改。

## obd cluster tenant decouple

使用该命令可解除主备关系，将一个备租户转换为独立的普通租户。
...
选项 `-p` 为 `--tenant-root-password`，配置为备租户 root 用户的登录密码。备租户 root 用户的登录密码默认与主租户保持一致。

...
使用该命令可以接管非 OBD 部署的集群。具体操作方法可参见 [使用 OBD 接管集群](../400.user-guide/300.command-line-operations/450.takeover-cluster.md)。

```shell
obd cluster takeover <deploy name> 

# example
obd cluster takeover test -h 10.10.10.1 -P 2881 -p ****** -ssh-user=admin --ssh-key-file=/home/admin/.ssh/id_rsa
```
...
| 选项名  |  是否必选  | 数据类型 | 默认值| 说明 |
|--------|------------|---------|-------|-----|
| -h/--host | 是     | string    | 127.0.0.1 | 待接管的 OceanBase 集群的连接 IP。 |
| -P/--mysql-port | 是 | int    | 2881 |  待接管的 OceanBase 集群的连接端口。     |
| -p/--root-password | 否 | string | 默认为空 | OceanBase 集群中 root@sys 用户的登录密码。 |

...
| -o/--optimize         | 是     | string     | 默认与集群场景保持一致  | 设置租户调优场景，默认租户场景与集群场景保持一致。可选值如下：<ul><li><code>express_oltp</code>：互联网应用，适合简单的 CRUD SQL、大量点查、高并发场景。</li><li><code>complex_oltp</code>：传统行业应用，适合有复杂查询、大量 PL 和批作业的场景。</li><li><code>olap</code>：适合在线分析处理和数仓场景。</li><li><code>htap</code>：适合 TP 增强混合负载场景。</li><li><code>kv</code>：OBKV 接口定义的 HBase 兼容和 Key-Value 表模型访问模式，适合单分区访问，无 SQL 层场景。</li></ul>。<main id="notice" type='notice'><h4>注意</h4><p>仅在部署的 OceanBase 数据库为 V4.3.0 或之后版本时，支持使用该选项设置租户调优场景。</p></main>  |
...
* `express_oltp`：互联网应用，适合简单的 CRUD SQL、大量点查、高并发场景。

* `complex_oltp`：传统行业应用，适合有复杂查询、大量 PL 和批作业的场景。

* `olap`：适合在线分析处理和数仓场景。

* `htap`：适合 TP 增强混合负载场景。

* `kv`：OBKV 接口定义的 HBase 兼容和 Key-Value 表模型访问模式，适合单分区访问，无 SQL 层场景。

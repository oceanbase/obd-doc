...
| -p/--tenant-root-password | 否  | string  | 空   | 主租户的 `root` 用户登录密码，用于连接主租户创建 `standbyro` 用户。当主租户下未创建过 `standbyro` 用户时，该选项为必选项。   |
<!-- 看 create-standby 命令中介绍 -p 是为了创建 standby 用户，如果已手动创建 standby 用户，那是不是就可以不配置 -p，改而配置--standbyro-password，
如果这样理解是正确的，那切换同步方式时，如果主租户中已经手动创建过 standby 用户，是不是单纯配置 -p 选项也不行啊  -->

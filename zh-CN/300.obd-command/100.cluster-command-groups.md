...
<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>自 obd V3.2.0 起，该命令支持为租户设置用户及密码，创建租户后的连接操作可参见《OceanBase 数据库》文档 <a href='https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000000218476'>连接 OceanBase 数据库</a> 章节。</p>
</main>
...
| -u/--username     |           | string    | root        | 租户下的用户名，配置后会自动创建该用户。           |
<!-- 如果设置为了非 root 值,如 test,还会为创建的租户自动创建 root 用户么 -->
| --password            |        | string   | 无      | 为 `-u`/`--username` 选项配置的用户设置密码。     |
<!-- 确认下是 -u 对应用户名的密码不, 如果创建的是 test 用户,就算设置了 --password 后 root 用户密码也是空的不 -->

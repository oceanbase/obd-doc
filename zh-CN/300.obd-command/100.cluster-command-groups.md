...
| -d/--data_backup_uri  | 否  | string  | 主租户备份时设置的数据备份文件的目录路径   |  设置存储数据备份文件的目录路径，创建备租户时会从该路径中获取主租户的数据备份信息。  <main id="notice" type='explain'><h4>说明</h4><p>当备租户访问不到主租户配置的备份信息时需要指定该选项，如：主备租户跨机房时，备租户无法访问主租户的 NFS，此时可以将数据备份文件复制到备租户可以访问的路径，并配置该选项。</p></main>     |
| -a/--archive_log_uri  |  否  | string   | 主租户备份时设置的日志归档文件的目录路径   | 设置存储日志归档文件的目录路径，创建备租户时会从该路径中获取主租户的日志归档信息。 <main id="notice" type='explain'><h4>说明</h4><p>当备租户访问不到主租户配置的备份信息时需要指定该选项，如：主备租户跨机房时，备租户无法访问主租户的 NFS，此时可以将日志归档文件复制到备租户可以访问的路径，并配置该选项。</p></main>      |
<!-- | --storage-info | 否  | string   |  无  | 当使用 OSS 作为备份介质时，通过该选项指定 OSS 的 access_key 信息。 |--替换为下面两个 -->
| --access_id  | 否  | int  | 无  | 当使用 OSS 或 S3 作为备份介质时，通过该选项指定 access_id。 |
| --access_key | 否  | string  | 无  | 当使用 OSS 或 S3 作为备份介质时，通过该选项指定 access_key。 |
<!-- 待优化
1. 如果 uri 中直接配置了 access_id 和 key 会报错么 -->
...
| -z/--zone-list        | 否    | string | 空                        | 指定租户的 Zone 列表，多个 Zone 用英文逗号 `,` 间隔。为空时等于集群全部 Zone。                     |
| --primary-zone        | 否    | string | RANDOM                   | 租户的主 Zone。                                                             |
| --time-zone  |  否  | varchar    | +08:00   | 设置待创建租户会话使用的时区，支持 `+08:00` 的 offset 形式和 `Asia/Shanghai` 的地域形式取值。 | ----这个是那一版本新增的啊
| --replica-num         | 否    | int    | 0                        | 租户副本数。为 `0` 时等于 Zone 的数目。                                              |
| --locality            | 否    | string | 空                        | 描述副本在 Zone 间的分布情况，如：F@z1,F@z2,F@z3,R@z4 表示 z1, z2, z3 为全功能副本，z4 为只读副本。 |
| -o/--optimize         | 否     | string     | 默认与集群场景保持一致  | 设置租户负载类型，未设置的情况下，会提供交互式选项供用户选择负载类型。可选值如下：<ul><li><code>express_oltp</code>：适用于贸易、支付核心系统、互联网高吞吐量应用程序等工作负载。没有外键等限制、没有存储过程、没有长交易、没有大交易、没有复杂的连接、没有复杂的子查询。</li><li><code>complex_oltp</code>：适用于银行、保险系统等工作负载。他们通常具有复杂的联接、复杂的相关子查询、用 PL 编写的批处理作业，以及长事务和大事务。有时对短时间运行的查询使用并行执行。</li><li><code>olap</code>：适用于实时数据仓库分析场景。</li><li><code>htap</code>：适用于混合 OLAP 和 OLTP 工作负载。通常用于从活动运营数据、欺诈检测和个性化建议中获取即时见解。</li><li><code>kv</code>：适用于键值工作负载和类似 Hbase 的宽列工作负载，这些工作负载通常具有非常高的吞吐量并且对延迟敏感。</li></ul><main id="notice" type='notice'><h4>注意</h4><p>仅在部署的 OceanBase 数据库为 V4.3.0 或之后版本时，支持使用该选项设置租户负载类型。</p></main>  |

## obd cluster tenant switch-log-source
<!-- 日志归档和备份恢复 之间的关系是什么样的？ -->
使用该命令可切换主备租户的同步方式。
<!-- 基于网络的切换为基于归档时，需要先执行归档操作 -->
```shell
obd cluster tenant switch-log-source <deploy name> <tenant name>

# example
obd cluster tenant switch-log-source test-standby tenant_a
```
...
|-----------------------|------|--------|--------------------------|------------------------------------------------------------------------|
| --type  | 否  | string   | SERVICE  | 指定备租户同步数据方式，可配置为 `LOCATION`（基于归档）或 `SERVICE`（基于网络）。 |
| -a/--archive_log_uri  |  当 `--type` 配置为 `LOCATION` 时为必选项   | string   | 主租户备份时设置的日志归档文件的目录路径   | 设置存储日志归档文件的目录路径，创建备租户时会从该路径中获取主租户的日志归档信息。 <main id="notice" type='explain'><h4>说明</h4><p>当备租户访问不到主租户配置的备份信息时需要指定该选项，如：主备租户跨机房时，备租户无法访问主租户的 NFS，此时可以将日志归档文件复制到备租户可以访问的路径，并配置该选项。</p></main>      |
<!-- | --storage-info | 否  | string   |  无  | 当使用 OSS 作为备份介质时，通过该选项指定 OSS 的 access_key 信息。 |--应该会同步替换为下面两个 -->
| --access_id  | 否  | int  | 无  | 当使用 OSS 或 S3 作为备份介质时，通过该选项指定 access_id。 |
| --access_key | 否  | string  | 无  | 当使用 OSS 或 S3 作为备份介质时，通过该选项指定 access_key。 |
<!-- 待优化
1. 如果 uri 中直接配置了 access_id 和 key 会报错么 -->
...
<!-- | --unlimited | 否  | bool | 无 | 设置是否连续回放存档的源日志，无需传值，命令中出现该选项即表示设置连续回放存档的源日志。 |---确认是否需要传值 -->
<!-- 优化选项介绍，这里应该不是 指定恢复的xxx吧，毕竟是在设置日志同步 -->

...
| --log-disk-size       | 否    | int    | 0                        | 指定租户的 Unit 日志盘大小，最小值为 `2G`。为 `0` 时使用集群剩余全部 LOG_DISK_CAPACITY，可使用 root 用户登录集群 sys 租户后，执行 `SELECT * FROM oceanbase.GV$OB_SERVERS;` 命令查看 LOG_DISK_CAPACITY 大小。           |
...
可通过 `-o`/`--optimize` 对指定的租户进行调优，默认租户负载类型与集群类型保持一致。未配置的情况下，会提供交互式选项供用户选择负载类型，可配置的值如下：

...
<!-- | -V/--version       | 是    | string |  空                    | 需要升级的组件的目标版本号。          | 支持升级 OMS 后，-V 选项还是必选么 -->
<!-- | --tag      | 可选 | string  | 无 | 指定升级目标版本的 tag，可执行 `docker images` 查看（`TAG` 字段）。  |
| -i/--image-name   | 可选  | string  | 无 | 指定升级目标版本的 Docker 镜像名，可执行 `docker images` 查看（`REPOSITORY` 字段）。  |-->
...
<!-- | --oms-backup-path  | 否 | string  |   | 升级 OMS 时设置备份元信息数据的目录。 |--确认是否有默认值 -->
| --dob/--disable-oms-backup  | 否 | bool | false  | 选项无需传值，配置即表示禁止备份 OMS 元信息数据。  |
<!-- 没有用于指定在线升级方式时，存放升级文件路径 的选项 -->

...
| --tag      | 可选 | string  | 无 | 指定升级目标版本的 tag，可执行 `docker images` 查看（`TAG` 字段）。 <main id="notice" type='explain'><h4>说明</h4><p>升级 OMS 时，该选项为必选项。</p></main> |
| -i/--image-name   | 可选  | string  | 无 | 指定升级目标版本的 Docker 镜像名，可执行 `docker images` 查看（`REPOSITORY` 字段）。<main id="notice" type='explain'><h4>说明</h4><p>升级 OMS 时，该选项为必选项。</p></main>  |
...
| --min_cpu         |  否   | int   | 无   |  租户可用最小 CPU 数，最小值为 `1`。未设置时默认等同于 `--max-cpu`。                                   |

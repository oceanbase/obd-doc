...
| --wp/--with-parameter | 否    | bool   | false | 带参启动。该选项无需传值，命令中配置即表示根据配置文件中的配置启动。  <main id="notice" type='explain'><h4>说明</h4><p>节点第一次启动时，无论是否配置该选项，均按带参启动处理。</p></main>                                                              |
| -S/--strict-check         | 否    | bool   | false | 部分组件在启动前会做相关的检查。检查不通过时，obd 将发出告警，不会强制停止流程。</br>使用该选项可开启检查失败报错直接退出。建议开启，可以避免一些资源不足导致的启动失败。                |
| --sn/--service-names  | 否  | string  | 空 | PowerRAG 要启动的服务列表，多个服务之间使用英文逗号（`,`）间隔。 |
...
| -V/--version       | 否    | string |  空                    | 需要升级的组件的目标版本号。<main id="notice" type='explain'><h4>说明</h4><p>升级非 OMS 的组件时，该选项为必选项。</p></main>        |
| --tag      | 否 | string  | 无 | 指定升级目标版本的 Tag，可执行 `docker images` 查看（`TAG` 字段）。 <main id="notice" type='explain'><h4>说明</h4><p>升级 OMS 时，该选项为必选项。</p></main> |
| -i/--image-name   | 否  | string  | 无 | 指定升级目标版本的 Docker 镜像名，可执行 `docker images` 查看（`REPOSITORY` 字段）。<main id="notice" type='explain'><h4>说明</h4><p>升级 OMS 时，该选项为必选项。</p></main>  |
| --skip-check       | 否    | bool   | false                  | 跳过可以跳过的检查。                  |
| --usable           | 否    | string | 空                     | 升级中使用的镜像 hash 列表，用 `,` 间隔。若待升级组件存在多个同版本的镜像，需要使用该选项指定具体镜像的 hash 值。 |
...
| --oms-backup-path  | 否 | string  | `~/oms/meta_backup_data`  | 升级 OMS 时设置备份元信息数据的目录。 |
| --dob/--disable-oms-backup  | 否 | bool | false  | 选项无需传值，配置即表示禁止备份 OMS 元信息数据。  |
...
| --standbyro-password | 否 | string  | 随机字符串  | 主租户提供给备租户的用户（`standbyro`）对应的密码，不允许为空。若之前已经创建过 `standbyro` 用户（非 create-standby 和 switch-log-source 命令创建），该选项为必选项，并指定为已存在的 `standbyro` 用户密码。 <main id="notice" type='explain'><h4>说明</h4><p>standbyro 用户密码中不允许包含：英文单引号（<code>'</code>）、英文双引号（<code>"</code>）、反引号（<code>`</code>）、英文分号（<code>;</code>）及空格（<code> </code>）。</p></main>   |
| --standby_archive_log_uri | 否  | string | 空  | 备租户存档日志的存储目录路径。在配置 OSS 作为存储介质，且 obd 无法自动获取 OSS 路径时（如主备租户所在集群为商业版 OceanBase 集群），需通过该选项指定存储目录路径。 |
|  --primary_archive_log_uri | 否  | string | 空  | 主租户存档日志的存储目录路径。在配置 OSS 作为存储介质，且 obd 无法自动获取 OSS 路径时（如主备租户所在集群为商业版 OceanBase 集群），需通过该选项指定存储目录路径。 |
...
| --unit-num                | 否   | int    |  1     | 租户在 Zone 上的 unit 数量。                                                                   |
| --replica-type            | 否   | string | FULL  | 设置租户在 Zone 上的副本类型，有如下两种取值：<ul><li><code>FULL</code>：表示全功能型副本</li><li><code>READONLY</code>：表示只读型副本</li></ul> |
| -p/--primary-zone         | 否   | string   | random | 待恢复的租户的 Primary Zone。                                               |
| -T/--timestamp            | 否   | time.time  |  无      | 指定恢复时间戳，设置后表示恢复到该时间点为止，且包括该 timestamp 位点。                |
| -S/--scn                  | 否   | int      |  0     | 指定恢复的 SCN，设置后表示恢复到该 SCN 位点为止，且包括该 SCN 位点。                      |
| -s/--ha-high-thread-score | 否   | int      | 10     | 用于设置高可用高优先级线程的当前工作线程数，取值范围：[0, 100]。                    |
| -c/--concurrency          | 否   | int      |  `${max-cpu}`      | 设置数据恢复的并发度，如果不指定，则默认等于该租户被分配的 MAX_CPU 数。              |
| -D/--decryption           | 否   | []string  |  无      | 备份集设置的密码，如设置了多个密码需写出全部的密码，并使用英文逗号（`,`）间隔。                                |
| -k/--kms-encrypt-info     | 否   | string | 无      | 密钥管理信息，如果未加密或恢复时可以访问原来的密钥管理服务，则可以不设置。              |
| --memory-size    | 否  |  string   | 无  | 设置待恢复租户的可用内存，建议尽量与源租户保持一致。未设置时会自动获取集群剩余可用资源创建待恢复租户。  |
| --max-cpu        | 否  | string    |  无  | 设置待恢复租户的最大可用 CPU，最小值为 1，建议尽量与源租户保持一致。未设置时会自动获取集群剩余可用资源创建待恢复租户。  |
| --min-cpu         |  否   | int   | 无   |  租户可用最小 CPU 数，最小值为 `1`。未设置时默认等同于 `--max-cpu`。                                   |
| --max-iops        |  否   | int   | 无   | 租户 IOPS 上限，最小值为 1024，指定的值要求满足 `--max-iops` >= `--min-iops`。如果不指定，则系统将根据以下规则自动计算 `--max-iops` 的值：<ul><li>如果已指定 <code>--min-iops</code> 的值，则 <code>--max-iops</code> 的值等于 <code>--min-iops</code> 的值。</li><li>如果 <code>--min-iops</code> 也未指定，则 <code>--max-iops</code> 和 <code>--min-iops</code> 的值均为 <code>INT64_MAX</code>。</li></ul>                            |
| --min-iops        |  否   | int   | 无   | 租户 IOPS 下限，最小值为 1024。如果不指定，则系统将根据以下规则自动计算 `--min-iops` 的值：<ul><li>如果已指定 <code>--max-iops</code> 的值，则 <code>--min-iops</code> 的值等于 <code>--max-iops</code> 的值。</li><li>如果 <code>--max-iops</code> 也未指定，则 <code>--min-iops</code> 和 <code>--max-iops</code> 的值均为 <code>INT64_MAX</code>。</li></ul>                    |
| --log-disk-size   |  否   | int   | 无   | 指定租户的 Unit 日志盘大小。未设置时默认为 3 倍的内存规格值。最小值为 `2G`。           |
...
| -V/--version      | 是    | string | 3.1.1                     | 指定待接管 OceanBase 集群的 OCP 版本号，用于检查 OCP 版本是否满足接管要求（需为 V3.1.1 或之后版本）。                                       |

...
| -f/--force-kil       | 否    | bool  | False      | 检查到工作目录下有运行中的进程时，强制停止进程。使用该选项会强制停止运行中的进程，强制执行 `obd cluster upgrade`。                         |
...
使用该命令可为主租户创建备租户，该命令仅对 OceanBase 数据库有效。该命令会自动创建资源单元和资源池，您不需要手动创建。

```shell
obd cluster tenant create-standby <standby-deployname> <primary-deployname> <primary-tenant> [options] 

# example
obd cluster tenant create-standby test-standby test-primary tenant_a
```

参数 `standby-deployname` 为备租户所在集群的部署集群名；`primary-deployname` 为主租户所在集群的部署集群名；`primary-tenant` 为主租户的租户名。

选项说明见下表：

|          选项名          | 是否必选 |  数据类型  |           默认值            |                                   说明                                   |
|-----------------------|------|--------|--------------------------|------------------------------------------------------------------------|
| --primary-tenant-root-password | 否  | string  | 空   | 主租户的 root 用户登录密码，当主租户所在集群未创建过备集群时，该选项为必选项。   |
| --standbyro-user-password | 否 | string  | 随机字符串  | 主租户提供给备租户的用户名（`standbyro`）对应的密码，若之前已经创建过 standbyro 用户（非 create-standby 命令创建），该选项为必选项。   |
| -t/--tenant-name  | 否  | string   | 与主租户一致  | 指定备租户名。 |
...
使用该命令可在主租户可用的情况下，将主租户与其中一个备租户交换角色，并保证数据无损。

```shell
obd cluster tenant switchover <deplo yname> <tenant>

# example
obd cluster tenant switchover test-standby tenant_a
```

参数 `deploy name` 为部署集群名，此处需填写备租户所在的集群名；`tenant` 为备租户的租户名。

## obd cluster tenant failover

使用该命令可在主租户不可用的情况下，将一个备租户转换为主租户。

```shell
obd cluster tenant failover <deploy name> <tenant>

# example
obd cluster tenant failover test-standby tenant_a
```
...
```shell
obd cluster tenant decouple <deploy name> <tenant>

# example
obd cluster tenant decouple test-standby tenant_a
```
...
| -t/--tenant-name       | 否    | string  | 空      | 进行租户名过滤，配置后仅展示配置的租户信息。                        |
...
| -f/--force-kil       | 否    | bool  | False      | 检查到工作目录下有运行中的进程时，强制停止进程。使用该选项会强制停止运行中的进程，强制执行 `obd cluster tenant drop`。                         |

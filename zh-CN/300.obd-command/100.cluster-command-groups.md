...
可通过 `-o`/`--optimize` 对指定的租户进行调优，默认租户场景与集群场景保持一致。可配置的值如下：
...
删除 oceanbase-ce 组件时，您可通过选项 `--igs/--ignore-standby` 设置当检查到集群中的租户在其他集群中存在备租户时，强制执行 `obd cluster component del` 命令。oceanbase-ce 组件删除后，备租户将无法使用。

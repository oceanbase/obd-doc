...
<main id="notice" type='notice'>
  <h4>注意</h4>
  <p>使用本节命令执行测试时，待测试的集群需是被 obd 管理的集群，否则 obd 无法获取集群的信息。obd 部署的集群默认由 obd 管理，非 obd 部署的集群可执行接管操作进行接管，具体操作可参见 <a href="../400.user-guide/300.command-line-operations/400.takeover-cluster/450.takeover-cluster.md">使用 obd 接管集群</a>。</P>
</main>
...
| --mode    | 否    | string   | both   | 指定测试模式，可配置的值及含义如下：<ul><li><code>mysql</code>：运行非 <code>_oracle</code> 结尾的测试用例。</li><li><code>oracle</code>：仅运行以 <code>_oracle</code> 结尾的测试用例。</li><li><code>both</code>：运行所有测试用例。</li></ul>   |
...
<main id="notice" type='explain'>
  ...
  <li>
  <p>本节以在线安装 ob-sysbench 为例，若您的机器无法连接外部网络，建议使用可连接外部网络的机器访问 <a href="https://mirrors.oceanbase.com/development-kit/">链接</a>，根据系统版本选择对应版本下载后上传到执行测试的机器，并使用 rpm 命令进行解压安装。</P>
  </li>
  </ul>
</main>
...
| --script-name         | 否    | string | oltp_point_select.lua             | 要执行的 sysbench 脚本名，支持配置多个脚本名，多个脚本名之间使用英文逗号（`,`）分隔。                                                                                         |
...
| --threads             | 否    | int    | 16                           | 启动的线程数量，支持配置多个值，多个值之间使用英文逗号（`,`）分隔。                                                                                                   |
...
| --rand-type           | 否    | string | special                            | 访问数据时使用的随机生成函数。取值可以为 special、uniform、gaussian 或 pareto。                     |
...
| -O/--optimization     | 否    | int    | 1                            | 自动调优等级。不同取值的含义如下：<ul><li>设置为 <code>0</code> 时表示关闭自动调优。</li><li>设置为 <code>1</code> 时会修改不需要重启生效的调优参数。</li><li>设置为 <code>2</code> 时会修改所有的调优参数，并根据需要重启集群使调优生效。</li></ul>     |
...
<main id="notice" type='explain'>
  ...
  <li>
  <p>本节以在线安装 obtpch 为例，若您的机器无法连接外部网络，建议使用可连接外部网络的机器访问 <a href="https://mirrors.oceanbase.com/development-kit/">链接</a>，根据系统版本选择对应版本下载后上传到执行测试的机器，并使用 rpm 命令进行解压安装。</P>
  </li>
  </ul>
</main>
...
| -S/--skip-cluster-status-check | 否   | bool  | false      | 开启时跳过集群状体检查，允许测试的时候有节点处于离线状态。   |
| -O/--optimization     | 否    | int    | 1                            | 自动调优等级。不同取值的含义如下：<ul><li>设置为 <code>0</code> 时表示关闭自动调优。</li><li>设置为 <code>1</code> 时会修改不需要重启生效的调优参数。</li><li>设置为 <code>2</code> 时会修改所有的调优参数，并根据需要重启集群使调优生效。</li></ul>     |
| --parallel              |  否   | int    | max_cpu * unit_count  | 用于设置加载数据的并行度。默认值中的 `max_cpu` 为租户的最大 CPU 个数，`unit_count` 为测试租户所用资源池中资源单元的个数。  |
| --direct-load           | 否    | bool   | false     | 控制是否开启旁路导入。   |
...
<main id="notice" type='explain'>
  ...
  <li>
  <p>本节以在线安装 obtpcc 为例，若您的机器无法连接外部网络，建议使用可连接外部网络的机器访问 <a href="https://mirrors.oceanbase.com/development-kit/">链接</a>，根据系统版本选择对应版本下载后上传到执行测试的机器，并使用 rpm 命令进行解压安装。</P>
  </li>
  </ul>
</main>

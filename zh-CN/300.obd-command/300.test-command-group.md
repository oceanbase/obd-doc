...
| --script-name         | 否    | string | point_select.lua             | 要执行的 sysbench 脚本名，支持配置多个脚本名，多个脚本名之间使用英文逗号（`,`）分隔。                                                                                         |
...
| -O/--optimization     | 否    | int    | 1                            | 自动调优等级。设置为 `0` 表示关闭自动调优；设置为 `1` 时会修改不需要重启生效的调优参数；设置为 `2` 时会修改所有的调优参数，并根据需要重启集群使调优生效。     |

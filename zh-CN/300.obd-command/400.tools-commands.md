...
* OBD_DISABLE_ALGORITHMS：用于指定待禁用的加密算法、密钥交换算法和消息认证码算法。改参数格式为 `{pubkeys: ["xxxx"]}`，其中 `xxxx` 为 SSH 服务的算法，多个算法之间使用英文逗号（`,`）隔开，如 `{pubkeys: ["rsa-sha2-512", "rsa-sha2-256"]}`。通过禁用特定的算法可以增强 SSH 连接的安全性。

...
参数 `deploy name` 为部署名，可以理解为配置文件的别名。
参数`SceneName`为故障场景名，可通过 `obd obdiag gather scene list` 来查看支持的场景

选项说明见下表：

| 选项名                 | 是否必选 | 数据类型   | 默认值                       | 说明                                                                |
|---------------------|------|--------|---------------------------|-------------------------------------------------------------------|
| --scene    | 是    | string | 默认为空               | 场景名，可以通过 `obd obdiag gather scene list` 查看当前版本支持哪些场景 |
| --from    | 否    | string | 默认为空        | 日志收集的开始时间，格式为: "yyyy-mm-dd hh:mm:ss"，需要加引号，例如 "1970-01-01 12:00:00"  |
| --to    | 否    | string | 默认为空        | 日志收集的结束时间，格式为: "yyyy-mm-dd hh:mm:ss"，需要加引号，例如 "1970-01-01 13:00:00"  |
| --since    | 否    | string | 默认为空          | 日志收集最近的某段时间，格式为: \<n> <m\|h\|d>，其中，n 表示待输入的时间数字，m 表示分钟，h 表示小时，d 表示天，例如 30m 表示收集最近 30 分钟的日志  |
...
使用该命令可以对 OBD 管理的 OceanBase 数据库进行特定故障场景的诊断信息分析，找出可能导致问题的原因。
...
参数 `deploy name` 为部署名，可以理解为配置文件的别名。
参数`scene_name`为故障场景名，可通过 `obd obdiag rca list` 来查看支持的场景

选项说明见下表：

| 选项名                 | 是否必选 | 数据类型   | 默认值                       | 说明                                                                |
|---------------------|------|--------|---------------------------|-------------------------------------------------------------------|
| --scene | 是 | string | 默认为空 | 需要进行的根因分析场景，可通过 `obd obdiag rca list` 查看支持的场景列表 |
| --report_path | 否 | string | 默认为 `./rca` | 根因分析结果保存路径 |
| --parameters | 否 | string | 默认为空 | 额外需要传递的参数 |
| --obdiag_dir |  否    | string |   $HOME/oceanbase-diagnostic-tool     | obdiag 的安装目录。 |
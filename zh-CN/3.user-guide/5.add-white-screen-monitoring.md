...
> **注意**
>
> 1. `scrape_configs` 中的 Prometheus 监控项中 `'localhost:9090'` 需要根据当前 Prometheus 的监听地址进行修改，如果当前 Prometheus 开启了认证，也需要对应的配置 `basic_auth`。这里说的监听地址为部署 Prometheus 的地址，即 Prometheus 配置中的 address 和 port 配置项。
>

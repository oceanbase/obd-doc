...
- 使用 OCP 接管集群时需要填写 sys 租户下 root 用户连接集群的密码，您需在配置文件中设置密码。部分配置文件示例如下：
    ...
    您需在 `root_password` 处填写密码。

## 配置 OBD 环境

### 检查是否满足条件

在使用 OCP 接管 OBD 部署集群前，您需先检查是否满足接管条件。如条件不满足，则可以根据提示参考下文进行修改。
...
该操作会生成运维任务，将 OCP 中现有 Oceanbase 集群的 proxyro 密码修改，同时 OBProxy 集群的密码也进行修改。
...
1. 为什么要修改 OCP 中的 proxyro 账号的密码？
   ...
   因为 OCP 中管理的 OBProxy 一般是通过 configserver 拉起的，设计上是可以连接多个 OceanBase 集群，但是 OBProxy 只能设置一个 proxyro 的用户密码，所以这个配置在 OCP 中是一个全局的配置，proxyro 用户的密码仅用来让 OBProxy 查询一些元数据，修改并不影响业务租户。

2. 切换新的 OBProxy，可以复用原来的机器吗？

   如果原来是多台 OBProxy，可以下线一个，然后在 OCP 中部署一个，再添加回 vip，一个一个依次进行。

...
| --force-delete                | 否    | bool   | false | 开启时，强制删除已注册的集群。         |
...
|            选项名            | 是否必选 |  数据类型  |  默认值  |                                                 说明                                                  |
|---------------------------|------|--------|-------|-----------------------------------------------------------------------------------------------------|
| -s/--servers              | 否    | string | 空     | 机器列表，后跟 `yaml` 文件中 `servers` 对应的 `name` 值，用 `,` 间隔。用于指定需启动的机器。如果组件下的机器没有全部启动，则 start 不会执行 bootstrap。 |
| -c/--components           | 否    | string | 空     | 组件列表，用 `,` 间隔。用于指定需启动的组件。如果配置下的组件没有全部启动，该配置不会进入 running 状态。                                          |
| -f/--force-delete            | 否    | bool   | false | 开启时，强制删除已注册的集群。         |
| --wop/--without-parameter | 否    | bool   | false | 无参启动。为 `true` 时表示启动的时候不带参数。节点第一次的启动时，不响应此选项。                                                                    |
...
选项 `-f` 为 `--force-kill`。作用为：检查到工作目录下有运行中的进程时，强制停止。重启前会做检查是有还有进程在运行中。这些运行中的进程可能是 `start` 失败留下的，也可能是因为配置与其他集群重叠，进程是其他集群的。但无论是哪个原因导致工作目录下有进程未退出，`obd cluster redeploy` 命令都会直接停止这些进程。

使用该选项会强制停止这些运行中的进程，强制执行 `obd cluster redeploy`，非必填项。数据类型为 `bool`。默认不开启。
...
|       选项名       | 是否必选 |  数据类型  | 默认值 |                              说明                               |
|-----------------|------|--------|-----|---------------------------------------------------------------|
| -s/--servers    | 否    | string | 空   | 机器列表，后跟 `yaml` 文件中 `servers` 对应的 `name` 值，用 `,` 间隔。用于指定需要停止的机器。 |
...
选项 `-f` 为 `--force-kill`。作用为：检查到工作目录下有运行中的进程时，强制停止。销毁前会做检查是有还有进程在运行中。这些运行中的进程可能是 `start` 失败留下的，也可能是因为配置与其他集群重叠，进程是其他集群的。但无论是哪个原因导致工作目录下有进程未退出，`obd cluster destroy` 命令都会直接停止这些进程。

使用该选项会强制停止这些运行中的进程，强制执行 `obd cluster destroy`，非必填项。数据类型为 `bool`。默认不开启。
...
| --usable           | 否    | string | 空                     | 升级中优先使用的镜像 hash 列表，用 `,` 间隔。 |
...
| --max-iops            | 否    | int    |<ul><li> 128（OceanBase 版本低于 V4.0.0） </li><li> 1024（OceanBase 为 V4.0.0 及以上版本）                     | 租户 IOPS 最多数量，该参数的取值范围根据 OceanBase 版本的不同分为以下两种情况。<ul><li>当 OceanBase 数据库版本低于 V4.0.0 时取值范围为 \[128,+∞)。</li><li>   当 OceanBase 数据库版本为 V4.0.0 及以上版本时取值范围为 \[1024,+∞)。 </li></ul>                                      |

...
<main id="notice" type='explain'>
  ...
  <main id="notice" type='explain'>
    <h4>说明</h4>
    <p>通过 OceanBase All in One 包安装 obd 时，<code>./install.sh</code> 脚本默认会尝试以管理员权限安装，若忘记密码或当前用户无 sudo 权限，可在输入密码处单击三次键盘 <code>Enter</code> 键，之后输入 <code>n</code> 选择使用当前用户进行安装。根据使用用户和权限的不同，有如下两种情况。</p>
    <ul>

...
若排查后发现并非以上两种原因导致，可根据报错信息，作进一步判断。
...
- 若您确认 Config Server 正确，可以在执行 `obd cluster start` 命令时加上 `-f` 参数覆盖已注册的集群。

- 您也可以执行 `obd cluster edit-config` 命令打开配置文件，修改配置项 `appname` 和 `cluster_id`。
...
### OBD-4305：There is not enough log disk for ocp meta tenant. (Avail: xxx, Need: xxx)

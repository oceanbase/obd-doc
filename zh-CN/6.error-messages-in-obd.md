...
对于情况 2，可先尝试再次启动组件，如果启动依旧失败，大概率是配置错误。可以参考 **OBD-2002** 解决。

对于情况 3，常见情况是用户直接执行 SQL 命令修改了密码，账号密码与配置文件中存储的不同导致 OBD 连接不到组件。该种情况下有以下两种解决办法。
...
对于情况 2，由于数据构建脚本是由 TPC 官网提供，可以先尝试重新跑测试，如果问题仍然存在请到官网 [问答区](https://ask.oceanbase.com/) 提问，会有专业人员为您解答。
...
可以先尝试重新跑测试，之后调整 terminals 等参数以减少测试压力。

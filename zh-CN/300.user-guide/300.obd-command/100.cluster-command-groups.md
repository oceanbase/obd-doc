...
| -c/--components       | 否    | string | 空     | 组件列表，用 `,` 间隔。用于指定需重启的组件。如果配置下的组件没有全部启动，该配置不会进入 running 状态。 |
...
|        选项名         | 是否必选 |  数据类型  |          默认值          |             说明              |
|--------------------|------|--------|-----------------------|-----------------------------|
| -f/--force         | 否    | bool   | false         | 开启时强制升级组件。   |
...
修改一个已部署的组件的仓库。新的仓库必须与当前仓库版本号相同。该命令在部署状态为 `running` 时，替换仓库后会使用无参启动，重新拉起组件。

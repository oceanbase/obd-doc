...
| 选项名 | 是否必选 | 数据类型 | 默认值 | 说明 |
|------- | ------- | ------- |------- |-----|
| -c/--component | 是 | string | 空 | 需要替换仓库的组件名。|
...
# example
obd cluster tenant create test -n obmysql --max-cpu=2 --max-memory=1G --max-disk-size=1G --max-iops=10000 --primary-zone=RANDOM --charset=utf8
```

...
2. 组件选择
   <!-- 验证选择 单机 模式时，组件展示和分布式时一样，需要确认预计是否会有变动 -->
   ...
   <!-- 勾选 OBAgent 时也自动勾选了 Prometheus、Grafana、AlertManager，应该不需要这样吧 -->
...
1. 集群配置
   ...
   | 数据目录 | 默认在 `/data/1` 下，支持自定义设置，需为以 <code>/</code> 开头的绝对路径，只能包含字母、数字和特殊字符（<code>-_:@/.</code>）。且需确保设置的目录为空，且 **节点配置** 中配置的用户拥有目录的访问和读写权限。  |
   | 日志目录 |  默认在 `/data/log1` 下，支持自定义设置，需为以 <code>/</code> 开头的绝对路径，只能包含字母、数字和特殊字符（<code>-_:@/.</code>）。且需确保设置的目录为空，且 **节点配置** 中配置的用户拥有目录的访问和读写权限。  |
...
您可在 **配置确认** 页面查看所有配置信息，若发现问题可单击 **上一步** 进行修改。确认无误后，单击 **下一步** 进行 **预检查** 页面，系统将验证您的环境及配置是否满足部署 OceanBase 集群的所有要求。

当选择 **部署模式** 为 **分布式**，且配置的 Zone 的个数大于等于 2 时，**配置确认** 页面将展示集群拓扑图，您可单击选择 **拓扑图** 进行查看。

![](200.use-ui-deploy-oceanbase-11)

...
| 负载类型 | 单击 **类型** 下的下拉框可自行选择 OceanBase 集群的负载类型，不同类型负载均衡下有对应描述，方便了解各类型的适用场景。<main id="notice" type='explain'><h4>说明</h4><p>仅在部署的 OceanBase 数据库为 V4.2.5 或之后版本时，支持设置集群负载类型。</p></main> |

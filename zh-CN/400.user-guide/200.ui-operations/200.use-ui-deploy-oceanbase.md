...
* 仅部署 OceanBase 数据库，至少需要 2vCPU、6 GB 内存、20 GB 磁盘的可用资源。需要注意，要求中的内存限制是指执行 `free -g` 命令输出中的 `available` 列对应的值。

* 部署 OceanBase 数据库及全部组件，至少需要 4vCPU、10 GB 内存、25 GB 磁盘的可用资源，推荐内存在 16 GB 以上。需要注意，要求中的内存限制是指执行 `free -g` 命令输出中的 `available` 列对应的值。

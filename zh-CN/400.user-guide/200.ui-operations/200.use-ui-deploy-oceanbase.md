...
* 单击输出的 Prometheus 组件和 Grafana 组件对应的连接串可对应跳转到 Prometheus 和 Grafana 的登录界面，通过部署界面展示的账号密码即可登录。
...
| 表明大小写敏感 | 可通过下拉框进行设置，支持设置为 `0` 或 `1`。<ul><li><code>0</code>：表名将按照指定的大小写形式进行存储，并以区分大小写形式进行比较。</li><li><code>1</code>：表名将按照小写形式进行存储，并以不区分大小写形式进行比较。</li></ul>  |
...
配置完成后单击 **下一步:创建业务租户**，系统将创建对应租户。创建完成后页面会展示业务租户信息，包含租户名、root 用户密码、连接字符串等信息。单击左下角 **退出** 按钮，即可退出创建流程。

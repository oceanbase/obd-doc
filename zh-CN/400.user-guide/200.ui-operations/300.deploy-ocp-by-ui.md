...
1. 服务配置
   ...
   | 软件路径 | OCP 的安装路径，默认在部署用户的家目录下创建 `ocp` 目录，可自定义修改，您需拥有对应目录的读写权限。<main id="notice" type='explain'><h4>说明</h4><p>若本地仓库中存在 OceanBase 数据库和 ODP 安装包，成功部署 OCP 后，OBD 会把本地仓库中存在的相关包上传到该目录下。</p></main> |

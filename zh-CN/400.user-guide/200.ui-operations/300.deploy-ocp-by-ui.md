...
1. 服务配置
   ...
   | 参数  | 说明  |
   |-------|-------|
   | Admin 密码 | 配置 OCP 管理员账号的登录密码，可自定义输入，也可单击 **随机生成**，由 OBD 生成随机字符串。密码需要满足以下复杂度条件：<ul><li>长度为 8~32 位字符</li><li>包含以下四种类型字符中至少三种：数字（0\~9）、大写字母（A\~Z）、小写字母（a\~z）和特殊字符（<code>~!@#%^&*_-+=\|(){}[]:;,.?/$`'"<>\\</code>）</li></ul></br>单击 **复制密码** 可复制配置的 Admin 密码。|

...
部署 obconfigserver 时，您可以选择单独部署，也可以和 OceanBase 数据库以及 ODP 一起部署，本文以合并部署为例介绍如何使用命令行部署 obconfigserver，并简单介绍部署 obconfigserver 后如何使用。
...
本节介绍如何和 OceanBase 数据库以及 ODP 一起部署 obconfigserver。一起部署的情况下，启动成功后该 OceanBase 数据库会被注册到 obconfigserver 中。

<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>单独部署 obconfigserver 时，您只需将 OceanBase 数据库和 ODP 的组件配置信息删去即可。</p>
</main>

### 步骤一：修改配置文件

若您要同时部署 OceanBase 数据库、ODP 和 obconfigserver，只需在部署 OceanBase 数据库和 ODP 的配置文件下新增 ob-configserver 组件信息，并在 OceanBase 数据库和 ODP 的组件下配置依赖 ob-configserver。
...
3. ODP 组件
   ...
   配置文件中各配置项含义可参见 [配置文件说明](../100.configuration-file-description.md)。您需注意请勿配置 `rs_list` 配置项，否则 ODP 无法获取 obconfigserver 中注册的集群信息。
...
4. obconfigserver 组件
   ...
   * `vip_address` 和 `vip_port`：`vip_address` 和 `vip_port` 为负载均衡访问地址配置项。若要在 `servers` 下配置多个节点，您需使用负载均衡，请在配置好负载均衡后，填写负载均衡地址 IP 与端口。
...
3. 复制执行 ob-configserver 下的访问信息
   ...
   输出中 ObRootServiceInfoUrlList 中包含一起部署的 OceanBase 集群（obcluster）的集群信息，表示该集群已成功注册到 obconfigserver 中。您可再次请求 ObRootServiceInfoUrl 验证 obconfigserver 中是否可以解析出 OceanBase 集群的信息，命令如下。
...
部署 obconfigserver 后，您可通过修改 OceanBase 集群配置文件或执行 SQL 命令将 OceanBase 集群注册到 obconfigserver 中，通过指定 obconfigserver 地址启动的 ODP 可以访问该 obconfigserver 中注册的所有 OceanBase 集群。

本节介绍如何将 OceanBase 数据库注册到 obconfigserver 中，以及如何通过指定 obconfigserver 地址启动 ODP。

### 注册 OceanBase 集群

本节介绍如何将 OceanBase 集群注册到 obconfigserver 中，有修改 OceanBase 集群配置文件和执行 SQL 命令两种方式。
...
3. 执行如下命令验证 obconfigserver 中是否注册了该集群的信息。
...
4. 执行如下命令验证 obconfigserver 中是否可以解析出 OceanBase 集群的信息。
...
当 ODP 通过 obproxy_config_server_url 配置项启动后，可以使用 ODP 连接注册到 obconfigserver 中的集群。你可以通过如下命令查看 obconfigserver 中注册的 OceanBase 集群信息。

```shell
curl -s 'http://10.10.10.1:8080/services?Action=GetObProxyConfig' |jq .
```

输出如下，从输出中可以看到该 obconfigserver 中已注册 obcluster 和 test1 两个集群，此时通过 ODP 可成功连接这两个集群。
...
```shell
2022-11-16T09:52:14.47799+08:00 ERROR [12258,] caller=cmd/main.go:36:func1: start configserver failed: start obconfigserver: create configserver schema: sql/schema: reading schema information Error 1235: while parameter _ob_enable_prepared_statement is disabled, prepared statement not supported

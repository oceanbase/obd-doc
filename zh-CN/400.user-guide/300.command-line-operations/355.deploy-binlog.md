...
部署 Binlog 服务时，您可以选择单独部署，也可以和将 OceanBase 数据库作为 MetaDB 一起部署，本文分别介绍如何操作。

## 前提条件

* 您的机器中已安装 V3.1.0 或之后版本的 obd，建议安装最新版本，安装操作可参见 [安装 obd](../../200.quick-start/100.install-obd.md)。

* 当依赖 OceanBase 数据库部署 Binlog 服务时，OceanBase 数据库需为 V4.2.1.0 或以上版本。
...
启动 Binlog 服务依赖元数据库，本节分为 **部署 Binlog 服务及 MetaDB（OceanBase 数据库）** 和 **已有 MetaDB 部署 Binlog 服务** 两种方法介绍如何部署 Binlog 服务：

* **部署 Binlog 服务及 MetaDB（OceanBase 数据库）**：可在配置文件中同时配置 OceanBase 数据库和 Binlog 服务信息，启动 Binlog 服务时，会根据配置启动 OceanBase 数据库作为 Binlog 的元数据库。

* **已有 MetaDB 部署 Binlog 服务**：您需确保已存在可用作 Binlog 服务元数据库的 OceanBase 数据库或 MySQL 数据库。

:::tab

tab 部署 Binlog 服务及 MetaDB（OceanBase 数据库）
...
5. 部署 Binlog 服务
   ...
   命令里的 `obbinlog` 为 Binlog 服务的部署名，您可根据实际情况自定义部署名。部署成功后命令行会输出对应的启动命令，输出如下：
...
6. 启动 Binlog 服务
   ...
   <main id="notice" type='explain'>
     <h4>说明</h4>
     <p>依赖 OceanBase 数据库启动 Binlog 服务时，obd 会自动在 OceanBase 数据库中创建 <code>binlog</code> 租户，并将 <code>root@binlog</code> 用户充当 Binlog 服务元数据库的连接用户。</p>
   </main>
...
本节以 obd 中 Binlog 服务的部署名为 `obbinlog` 为例介绍如何使用 obd 管控 Binlog 服务。
...
* 停止 Binlog 服务
  ...
  使用该命令停止 Binlog 服务不影响 Binlog 实例，未被停止的 Binlog 实例仍在运行中。
...
* 销毁 Binlog 实例
  ...
  该命令会停止 Binlog 服务及 Binlog 实例，并销毁 Binlog 服务目录。
...
* 升级 obbinlog
  ...
  obd 自 V3.2.0 起支持升级 obbinlog，且支持升级的目标版本为 V4.2.0 及之后版本。升级 obbinlog 后，后续新建或启动的 Binlog 实例将使用新版本的 obbinlog 程序包。如果您希望升级已有的 Binlog 实例，请参见官网 [更新和升级](https://www.oceanbase.com/docs/common-oblogproxy-doc-1000000002039626) 中 **轮转升级 Binlog 任务** 部分内容。
  <!-- 确认下是否只等同于下文中的 升级Binlog 服务
   https://www.oceanbase.com/docs/community-oblogproxy-doc-1000000001999439-->

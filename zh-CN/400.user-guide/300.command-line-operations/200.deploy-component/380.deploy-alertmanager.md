...
3. 配置 OceanBase 数据库
   ...
   * 配置文件中若未配置 `scenario`，obd 会提供交互式选项供用户选择负载类型。
     ...
     对于 OceanBase 数据库 V4.2.x 版本，该功能从 V4.2.5 开始支持；对于 OceanBase 数据库 V4.3.x 版本，该功能从 V4.3.0 开始支持；对于 OceanBase 数据库 V4.4.x 版本，该功能从 V4.4.0 开始支持。
...
5. 配置监控相关组件
   ...
   <main id="notice" type='explain'>
     <h4>说明</h4>
     <p>此处示例以接收类型为 <code>webhook</code> 为例，配置中的 webhook 地址需要根据您的实际接收端点进行修改。如需配置邮件、企业微信等其他通知方式，请参考 <a href="../100.configuration-file-description.md">配置文件说明</a> 中的 <b>Alertmanager</b> 部分的 <b>receivers</b> 介绍。</p>
   </main>

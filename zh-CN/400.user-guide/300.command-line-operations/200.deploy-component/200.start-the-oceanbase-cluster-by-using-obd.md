...
本文介绍如何使用 obd 部署单节点 OceanBase 集群。操作示例仅用于帮助了解 OceanBase 数据库，不建议生产环境中使用。
...
* 您的机器可以访问外部网络，或者 obd 本地镜像库中存在部署所需组件安装包（OceanBase 数据库、ODP 等）。
...
1. 填写用户信息
   ...
   <main id="notice" type='explain'>
     <h4>说明</h4>
     <p>默认在对应机器的当前同名用户下部署 OceanBase 集群，需保证输入的机器中包含当前同名用户。例如，使用 admin 用户执行命令进入交互式部署，此处输入的机器中需存在 admin 用户，密码也需填写 admin 用户的密码。</p>
   </main>
...
2. 修改系统参数
   ...
   配置节点信息后，若当前机器之前未执行过 `obd cluster init4env` 命令，该命令会获取节点的系统参数，并向您确认是否需要修改，具体如下：
   ...
   * 若您输入为 `y`，程序会开始修改对应系统参数，并输出如下提示。
     ...
     ```shell
     get system config ok
     10.10.10.1: ( fs.aio-max-nr,vm.max_map_count,fs.file-max,nofile ) have been successfully modified!
...
3. 配置集群信息
   ...
   <main id="notice" type='explain'>
      <h4>说明</h4>
      <p>脚本会根据机器的资源情况自动获取集群资源配置的取值范围和默认值，机器资源情况不同时，输出的取值范围和默认值也不相同。</p>
   </main>
   ...
   #选择部署的 OceanBase 数据库类型，仅在 obd 镜像库中存在多种类型态的 OceanBase 数据库（单机版、企业版、社区版）时展示
   Enter the cluster name (Default: myoceanbase; allowed characters: letters, numbers, and underscores):
       Multiple OceanBase packages found in the mirror. Please choose one:
       1) oceanbase-ce (Community)
       2) oceanbase-standalone (Business)
   Enter the number of the oceanbase type you want to use: 1
...
4. （可选）配置租户信息
   ...
   #设置待创建租户的可用日志磁盘大小
   Enter the tenant log disk size (Configurable Range[4, 22], Default: 4 , unit: G): 6
   #设置待创建租户的字符集
   ...
   #设置待创建租户中是否表名大小写敏感，仅在创建 MySQL 租户兼容模式时需要配置
      Please select case sensitivity for table names:
       1) Table names are stored as specified and compared case-sensitively
       2) Table names are stored in lowercase and compared case-insensitively
       3) Table names are stored as specified but compared case-insensitively
...
7. 确认配置
   ...
   data dir: /home/admin/myoceanbase/data
   log dir: /home/admin/myoceanbase/log
...
8. 部署成功
   ...
   Exec oceanbase-ce-4.3.5.1-101020012025061620.el7-fe7d924524546c9e0188a36194aa69332933e36c import_time_zone_info.py ok
   Exec oceanbase-ce-4.3.5.1-101020012025061620.el7-fe7d924524546c9e0188a36194aa69332933e36c import_srs_data.py ok
...
4. 连接集群
   ...
   ```shell
   obclient -h172.16.59.106 -P2881 -uroot@sys -p'******' -Doceanbase -A
   ```
...
2. 启动 OceanBase 集群
   ...
   | ip            | version | port | zone  | status |
   +---------------+---------+------+-------+--------+
   | 172.16.59.106 | 4.3.5.2 | 2881 | zone1 | ACTIVE |
   +---------------+---------+------+-------+--------+
   obclient -h172.16.59.106 -P2881 -uroot@sys -p'******' -Doceanbase -A
...
2. 查看集群具体状态
   ...
   Get local repositories and plugins ok
   Open ssh connection ok
   Connect to observer 172.16.59.106:2881 ok
   Wait for observer init ok
   +-------------------------------------------------+
   |                   oceanbase-ce                  |
   +---------------+---------+------+-------+--------+

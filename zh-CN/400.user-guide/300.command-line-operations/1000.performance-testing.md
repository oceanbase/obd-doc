...
3. 执行 TPC-H 测试
   ...
   * 示例中以待测试的集群名为 `test`，租户名为 `tpch_mysql` 为例，您需根据实际情况进行替换。同时，您也可根据自己的具体情况做一些参数上的调整。
...
2. 执行 TPC-C 测试
   ...
   * 示例中以待测试的集群名为 `test`，租户名为 `tpcc_mysql` 为例，您需根据实际情况进行替换。同时，您也可根据自己的具体情况做一些参数上的调整。
...
2. 创建测试脚本
   ...
   此处以脚本名为 `sysbench.sh` 为例，您可自定义测试脚本名。
   ...
   * 脚本中的 `<deploy_name>` 为待测试的集群名，需根据实际情况进行修改。您可执行 `obd cluster list` 命令查看集群名。

   * 脚本中的 `<tenant_name>` 为待测试的租户名，需根据实际情况进行修改。您可执行 `obd cluster tenant show <deploy_name>` 命令查看集群中存在的租户。

   * 示例脚本中大幅参数使用的是默认值，您可根据自己的具体情况做一些参数上的调整。详细的命令介绍可参见 [测试命令组](../../300.obd-command/300.test-command-group.md) 中 **obd test sysbench**。

3. （可选）配置脚本的执行权限

   若测试脚本无执行权限，您可执行如下命令为创建的脚本添加执行权限，否则执行测试时会报 `Permission denied`。

   ```shell
   chmod u+x sysbench.sh
   ```

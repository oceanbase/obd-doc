...
<main id="notice" type='explain'>
  ...
  <ul>
  <li>
  <p>使用 obd 命令进行性能测试时，待测试的集群需是被 obd 管理的集群，否则 obd 无法获取集群的信息。obd 部署的集群默认由 obd 管理，非 obd 部署的集群可执行接管操作进行接管。接管的详细操作可参见 <a href='../300.command-line-operations/450.takeover-cluster.md'>使用 obd 接管集群</a>。</p>
  </li>
  <li>
...
* OceanBase 数据库：需部署 OceanBase 数据库，并创建执行测试的租户及用户。部署 OceanBase 数据库的操作可参见 [通过图形化界面部署 OceanBase 集群](../../200.quick-start/300.use-ui-deploy-oceanbase.md)，创建租户可使用 `obd cluster tenant create` 命令，详细介绍可参见 [集群命令组](../../300.obd-command/100.cluster-command-groups.md) 中 **obd cluster tenant create** 部分内容。

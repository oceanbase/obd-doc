...
| regions.cm_url | 必选  | 无  | 重启进程  | 访问 OMS 集群管理服务的地址。可只填写 IP 和端口，obd 部署时会自动补全 `http://`，若 url 前缀为 `https://`，设置时需写全，如 `https://xx.xx.xx.xx:8088`。未部署 VIP 的情况下，端口需与 `cm_server_port` 配置保持一致。<main id="notice" type='explain'><h4>说明</h4><p>单节点部署时，通常配置为当前 OMS 机器 IP。不建议您使用 http://127.0.0.1:8088。</p></main> |

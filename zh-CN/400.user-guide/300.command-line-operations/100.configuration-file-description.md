...
* seekdb
  ...
  seekdb 是一款 AI 原生混合搜索数据库，在一个数据库中融合向量、文本、结构化与半结构化数据能力，并通过内置 AI Functions 支持多模混合搜索与智能推理。详细信息请参见 [了解 seekdb](https://www.oceanbase.ai/docs/zh-CN/seekdb-overview)。
...
* **scale_out**：该目录下的配置文件示例是为扩容和组件变更功能准备的，您可执行相关命令使用对应配置文件对组件进行扩容，或添加或移除对应组件。具体操作可参见 [扩容与组件变更](300.manage-cluster/900.expansion-and-component-changes.md)。
  ...
  <main id="notice" type='explain'>
    <h4>说明</h4>
    <p><b>scale_out</b> 目录下仅提供了部分组件的配置文件示例，目前 obd 支持除 obbinlog-ce、oms、seekdb、maas 和 powerrag 外所有组件的新增，以及除 oblogproxy、obbinlog-ce、oms、seekdb、maas 和 powerrag 外其他组件的扩容。</p>
  </main>

* **seekdb**：该目录下的配置文件示例是为部署 seekdb 准备的，您可参考提供的配置文件示例单独部署 seekdb。
...
此处以 `example/seekdb/seekdb-single-example.yaml` 文件为例介绍部署 seekdb 时需注意的配置项。通过配置文件部署 seekdb 流程可参见 [部署 seekdb](200.deploy-component/300.start-the-seekdb-by-using-obd.md) 中 **通过配置文件部署 seekdb** 部分。
...
您需关注的配置项如下表所示，详细的配置项介绍可参见 [seekdb 配置项](https://github.com/oceanbase/obdeploy/blob/master/plugins/seekdb/1.0.0/parameter.yaml)。

|  配置项  |  是否必选  |  默认值  |  说明     |
|----------|-----------|----------|----------|
| servers  | 必选      | 无        | 每台机器需要用 `- name: 机器标识名（换行）ip: 机器 IP` 指定，多个机器就指定多次，机器标识名不能重复。</br> 在机器 IP 不重复的情况下，也可以使用 `- <ip> （换行）- <ip>` 的格式指定，此时 `- <ip>` 的格式相当于 `- name: 机器 IP（换行）ip: 机器 IP`。 |
| home_path  | 必选      | 无       | seekdb 安装路径。 |
| data_dir  | 可选      | $home_path/store      | 设置存储 SSTable 等数据的目录，建议配置为独立的磁盘。 |
| redo_dir  | 可选      | 默认与 `data_dir` 相同       | 设置事务日志盘路径，建议配置为独立的磁盘，并确保有足够的磁盘空间（`log_disk_size` 设置）。 |
| mysql_port  | 必选      | 2881        | 设置 SQL 服务协议端口号，取值范围为 \[1025, 65535]，默认为 `2881`。 |
| rpc_port  | 必选      | 2882       | 设置远程访问的协议端口号，可用于为 seekdb 备实例同步数据，取值范围为 \[1025, 65535]，默认为 `2882`。 |
| obshell_port | 必选   | 2886      | 设置 seekdb 实例运维端口，取值范围为 \[1025, 65535]，默认为 2886。 |
| memory_limit  | 可选      | 1G       | 设置 seekdb 实例的内存。 |
| memory_hard_limit  | 可选  | 4G    | 用于控制 seekdb 的内存硬限制，取值为 `0` 时表示使用系统内存的 90% 作为内存硬限制。 |
| datafile_size  | 可选      | 无        | 设置数据盘的初始空间，未配置的情况下参照 seekdb 的处理逻辑。 |
| datafile_next_size  | 可选  | 无       | 设置数据盘自动扩容的步长，未配置的情况下参照 seekdb 的处理逻辑。 |
| datafile_maxsize  | 可选  | 20G     | 限制磁盘空间的最大可用上限，用于设置自动扩容。 |
| cpu_count  |  可选  |  无  | seekdb 可使用的 CPU 总数，如果设置为 `0`，将自动检测。  |
| max_syslog_file_count  | 可选      | 2        | 设置在回收日志文件之前可以容纳的日志文件数量。值为 `0` 时表示不自动清理。 |
| log_disk_size  | 可选      | max(1/2 * memory_limit , 2G)   | 用于设置 clog 文件可使用的磁盘空间上限，设置时需确保 `redo_dir` 配置的路径有足够的磁盘空间。 |
| root_password  | 可选      | 随机字符串      | seekdb 实例 root 用户的密码，建议设置复杂的密码。未设置的情况下会自动生成随机字符串，可在部署成功后通过 `obd cluster edit-config` 命令查看配置文件中对应的配置项获取密码。 |
| enable_auto_start  | 可选  | false     | 控制是否开启 seekdb 进程自启动功能，配置为 `true` 时，若 seekdb 节点重启，会自动拉起 seekdb 进程。<main id="notice" type='explain'><h4>说明</h4><p>配置该配置项时需确保部署环境非容器环境，且部署用户拥有 sudo 权限。</p></main>   |
| production_mode    | 必选     | true      | 控制是否设置待部署实例为生产模式，默认为 `true` 表示为生产模式。 |
| enable_rpc_service |  可选  | false  | 控制是否开启 RPC 服务。若要为当前 seekdb 实例部署备实例，需将该选项设置为 `true`。  |
| log_disk_utilization_threshold  |  可选  | 0  | 设置 `log_disk_size` 的使用百分比，默认值为 `0` 表示尽快回收，推荐设置为 `80`。当使用率超过该百分比时，会触发重做回收。 |

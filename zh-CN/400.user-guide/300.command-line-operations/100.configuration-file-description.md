...
此处以 `example/alertmanager-only-example.yaml` 文件为例介绍部署 Alertmanager 时需要注意的配置项。推荐通过 obd 白屏页面部署 OCP，详细操作可参见 [部署 OceanBase 集群](../200.ui-operations/200.use-ui-deploy-oceanbase.md)。
...
| data_dir  | 可选  | ${home_path}/data  | 数据存储目录，对应 Alertmanager 原生配置中的 `storage_path` 配置。 |
...
| data_retention | 可选  |  120h | 数据保存时长，超过设置值后数据将被删除。 |
| basic_auth_users | 可选  | 用户名 admin，密码随机字符串  | 设置访问 web 服务器的用户名和密码，明文配置账密，通过 `<username>: <password>` 格式配置。对应 Alertmanager 原生配置中的 `web.config.fil` 参数。  |
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>from</td>
      <td>global.smtp_from</td>
      <td>发件人的邮箱地址。</td>
    </tr>
    <tr>
      <td>smarthost</td>
      <td>global.smtp_smarthost</td>
      <td>发送电子邮件的 SMTP 主机。</td>
    </tr>
    <tr>
      <td>auth_username</td>
      <td>global.smtp_auth_username</td>
      <td>SMTP 身份验证信息。</td>
    </tr>
    <tr>
      <td>auth_password</td>
      <td>global.smtp_auth_password</td>
      <td>SMTP 身份验证信息，与 <code>auth_password_file</code> 互斥，两者仅支持配置其一。</td>
    </tr>
    <tr>
      <td>auth_password_file</td>
      <td>global.smtp_auth_password_file</td>
    ...
    <tr>
      ...
      <td>api_secret</td>
      <td>global.wechat_api_secret</td>
      <td>与微信 API 对话时使用的 API 键。</td>
    </tr>
    <tr>
      <td>corp_id</td>
      <td>global.wechat_api_corp_id</td>
      <td>用于身份验证的公司 ID。</td>
    </tr>
    <tr>
      <td>webex</td>

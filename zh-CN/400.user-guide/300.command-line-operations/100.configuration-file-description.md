...
| servers  | 节点信息，每台机器需要用 `- name: 机器标识名（换行）ip: 机器 IP` 指定，多个机器就指定多次，机器标识名不能重复。</br>在机器 IP 不重复的情况下，也可以使用 `- <ip> （换行）- <ip>` 的格式指定，此时 `- <ip>` 的格式相当于 `- name: 机器标识名（换行）ip: 机器 IP`。   |
...
oceanbase-ce:
  ...
  global:
    ...
    install_utils: true
    obindex_split_policy: DISTRIBUTED
...
| servers  | 必选      | 无        | 每台机器需要用 `- name: 机器标识名（换行）ip: 机器 IP` 指定，多个机器就指定多次，机器标识名不能重复。</br> 在机器 IP 不重复的情况下，也可以使用 `- <ip> （换行）- <ip>` 的格式指定，此时 `- <ip>` 的格式相当于 `- name: 机器标识名（换行）ip: 机器 IP`。 |
...
| enable_auto_start  | 可选  | false     | 控制是否开启 observer 进程自启动功能，配置为 `true` 时，若 OBServer 节点重启，会自动拉起 observer 进程。<main id="notice" type='explain'><h4>说明</h4><p>配置该配置项时需确保所用用户拥有 sudo 权限。</p></main>   |
...
| obindex_split_policy  | 可选 | DISTRIBUTED  | 控制新建的全局索引是否开启自动分区分裂功能。可设置为如下值：<ul><li><code>DISTRIBUTED</code>（默认值）：租户内有多节点资源可用时（比如租户的 Primary Zone 超过 1 个，或者租户的 Unit 超过 1个），新建全局索引时，默认开启自动分区分裂。</li><li><code>ALL</code>：新建全局索引时，默认开启自动分区分裂。</li><li><code>OFF</code>：新建全局索引时，跟随原先主表的自动分区分裂策略。</li></ul> <main id="notice" type='notice'><h4>注意</h4><p>仅在待部署的 OceanBase 数据库为社区版 V4.3.5 或之后版本时，支持使用该配置项控制自动分区分裂。</p></main> |
...
|  配置项  |  是否必选  |  默认值  |  说明     |
|----------|-----------|----------|----------|
| servers  | 必选      | 无        | 每台机器需要用 `- name: 机器标识名（换行）ip: 机器 IP` 指定，多个机器就指定多次，机器标识名不能重复。</br> 在机器 IP 不重复的情况下，也可以使用 `- <ip> （换行）- <ip>` 的格式指定，此时 `- <ip>` 的格式相当于 `- name: 机器标识名（换行）ip: 机器 IP`。<blockquote>**注意**</br>使用 OBAgent 监控 OceanBase 数据库时，servers 配置需和 OceanBase 数据库组件下的 servers 保持一致。</blockquote> |
...
|----------|-----------|----------|-------------|----------|
| servers  | 必选      | 无        | 重新部署    | 每台机器需要用 `- name: 机器标识名（换行）ip: 机器 IP` 指定，多个机器就指定多次，机器标识名不能重复。</br> 在机器 IP 不重复的情况下，也可以使用 `- <ip> （换行）- <ip>` 的格式指定，此时 `- <ip>` 的格式相当于 `- name: 机器标识名（换行）ip: 机器 IP`。 |

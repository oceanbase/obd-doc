...
| enable_auto_start  | 可选  | false     | 控制是否开启 observer 进程自启动功能，配置为 `true` 时，若 OBServer 节点重启，会自动拉起 observer 进程。<main id="notice" type='explain'><h4>说明</h4><p>配置该配置项时需确保部署用户拥有 sudo 权限，且部署环境非容器环境。</p></main>   |

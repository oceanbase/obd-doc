...
```
<!-- 您需关注的配置项如下表所示，详细的配置项介绍可参见 [OMS 配置项](https://github.com/oceanbase/obdeploy/blob/master/plugins/oms/1.0.0/parameter.yaml)。 等产品发布后更新链接-->
...
| drc_cm_heartbeat_db  | 必选   | 无  | 不支持修改   | 集群管理服务的心跳库的名称，obd 在部署 OMS 时会根据配置在元信息库中创建对应数据库。 |
| drc_cm_db  | 必选   | 无  | 不支持修改  | 集群管理服务的元数据库名称，obd 在部署 OMS 时会根据配置在元信息库中创建对应数据库。  |
| drc_rm_db  | 必选   | 无   | 不支持修改  | 管理控制台的数据库名称，obd 在部署 OMS 时会根据配置在元信息库中创建对应数据库。  |

...
* **scale_out**：该目录下的配置文件示例是为扩容和组件变更功能准备的，您可执行相关命令使用对应配置文件对组件进行扩容，或添加或移除对应组件。具体操作可参见 [扩容与组件变更](300.manage-cluster/900.expansion-and-component-changes.md)。
  ...
  <main id="notice" type='explain'>
    <h4>说明</h4>
    <p><b>scale_out</b> 目录下仅提供了部分组件的配置文件示例，目前 obd 支持除 obbinlog-ce 和 oms 外所有组件的新增，以及除 oblogproxy、obbinlog-ce 和 oms 外其他组件的扩容。</p>
  </main>
...
此处以 `example/maas/maas-only-example.yaml` 文件为例介绍部署 maas 时需注意的配置项。通过配置文件部署 maas 流程可参见 [通过命令行部署 oblogproxy](200.deploy-component/460.deploy-oblogproxy.md)。
...
| tag   | 可选      | 无    | 指定待部署组件 tag，可执行 `docker images` 查看（`TAG` 字段）。         |
| servers      | 必选      | 无              | 每台机器需要用 `- name: 机器标识名（换行）ip: 机器 IP` 指定，也可以使用 `- <ip>` 的格式指定，此时 `- <ip>` 的格式相当于 `- name: 机器 IP（换行）ip: 机器 IP`。 |
...
此处以 `example/powerrag/powerrag-only-example.yaml` 文件为例介绍部署 PowerRAG 时需注意的配置项。通过配置文件部署 PowerRAG 流程可参见 [通过命令行部署 oblogproxy](200.deploy-component/460.deploy-oblogproxy.md)。
...
<!-- | defualt_password | 可选 | 随机字符串 | 设置 OceanBase 、Redis、Elasticsearch 等服务的密码。建议使用强密码，包含大小写字母、数字和特殊字符。 |--这是用于干啥的密码啊；未配置的时候会生成随机字符串不 -->
| ob_tenant_name | 
<!-- | ob_tenant_password | 根据powerrag 文档来看，defualt_password 也用于设置 OB 集群下 powerrag 租户的 root 密码，obd 里给了这个配置文件，那 defualt_password 就不会设置 OB 租户的密码了吧 -->
| cache_scheme | 

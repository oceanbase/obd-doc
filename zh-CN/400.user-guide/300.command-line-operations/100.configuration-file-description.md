...
<!-- 您需关注的配置项如下表所示，详细的配置项介绍可参见 [MaaS 配置项](https://github.com/oceanbase/obdeploy/blob/master/plugins/oblogproxy/2.0.0/parameter.yaml)。 -->
...
| servers      | 必选      | 无              | 每台机器需要用 `- name: 机器标识名（换行）ip: 机器 IP` 指定，也可以使用 `- <ip>` 的格式指定，此时 `- <ip>` 的格式相当于 `- name: 机器 IP（换行）ip: 机器 IP`。 <main id="notice" type='explain'><h4>说明</h4><p>仅支持为 MaaS 配置单个节点。</p></main>|
...
此处以 `example/powerrag/powerrag-only-example.yaml` 文件为例介绍部署 PowerRAG 时需注意的配置项。
...
<!-- 您需关注的配置项如下表所示，详细的配置项介绍可参见 [PowerRAG 配置项](https://github.com/oceanbase/obdeploy/blob/master/plugins/oblogproxy/2.0.0/parameter.yaml)。 -->

|  配置项       |  是否必选  |  默认值  |  说明     |
|--------------|-----------|----------|----------|
| type   |  必选  | rpm  | 指定组件的类型，支持指定为 `rpm` 或 `docker`。部署 PowerRAG 时必须指定为 `docker`。  |
| version      | 可选      | 默认部署最新版本 | 指定待部署组件版本，通常情况下不需要指定。 |
| servers      | 必选      | 无              | 每台机器需要用 `- name: 机器标识名（换行）ip: 机器 IP` 指定，也可以使用 `- <ip>` 的格式指定，此时 `- <ip>` 的格式相当于 `- name: 机器 IP（换行）ip: 机器 IP`。<main id="notice" type='explain'><h4>说明</h4><p>仅支持为 PowerRAG 配置单个节点。</p></main> |
...
<!-- | default_password | 可选 | 随机字符串 | 设置 OceanBase 数据库服务的密码。建议使用强密码，包含大小写字母、数字和特殊字符。 |--需要确认描述 -->
| ob_tenant_name | 可选  |  powerrag |  obd 会根据配置自动在 OceanBase 数据库中创建同名租户。  |
<!-- | ob_tenant_password | 可选 | 随机字符串 | 配置 `ob_tenant_name` 对应租户的管理员用户（`root`）密码。 |根据powerrag 文档来看，default_password 也用于设置 OB 集群下 powerrag 租户的 root 密码，obd 里给了这个配置文件，那 default_password 就不会设置 OB 租户的密码了吧 -->

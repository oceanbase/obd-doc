...
本文以 **全组件部署(和OceanBase 一起部署)** 和 **仅部署ObLogProxy** 两种场景为例，介绍如何使用命令行部署 OBLogProxy。

<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>1. 本文默认所使用的机器可以连接公网，或者已配置好所需软件（OceanBase、ODP、OBLogProxy、Config Server 等）。配置所需软件的详细操作可参考 <a href="../../200.quick-start/100.install-obd.md"> 安装并配置 OBD</a> 中 <b>配置 OBD</b>。</p>
  <p>2. 全组件部署: 指当前环境中未部署任何 OceanBase 组件，所有组件（oceanbase-ce、obproxy-ce、ob-configserver、oblogproxy）均由 OBD 一次性进行部署, OBD 版本2.5.0 或更高版本。</p>
  <p>3. 仅部署ObLogProxy: 指当前环境中已存在 OceanBase 集群，需要使用 OBLogProxy 的服务，此时仅需要部署 OBLogProxy。如果使用 OBD 进行部署, OBD 版本需要为 2.9.0 或更高版本, 否则, 请参考文档 [OBLogProxy 安装](https://www.oceanbase.com/docs/community-oblogproxy-doc-1000000000531982) 进行部署</p>
  <p>4. 若OBLogProxy 为 Binlog Service 模式，需要部署 OBProxy 和 Config Server </p>
</main>
...
<main id="notice" type='explain'>
  <h4>说明</h4>
  <p>若使用 OBLogProxy 的 Binlog Service 服务，需要部署 OBProxy 和 Config Server。</p>
</main>
...
1. 将OBLogProxy rpm 加载到 OBD 仓库中

  1. 从 [下载中心](https://www.oceanbase.com/softwarecenter) 下载 最新“OceanBase 日志代理服务”
  假设当前最新版本为 oblogproxy-2.0.1-202000012024041520.el8.x86_64.rpm
  ```shell 
  obd mirror clone rpm -i oblogproxy-{version}.{arch}.rpm
  ```
...
4. 配置 obproxy-ce 和 ob-configserver 组件

   若无需使用 OBLogProxy 的 Binlog Service 服务，可不部署 OBProxy 和 Config Server
...
1. 部署 OBLogProxy 前，您需确认已安装 V2.9.0 或之后版本的 OBD。若您环境中 OBD 为 V2.9.0 之前版本，可参考 [常见问题](../../500.faq/100.faq.md) 中 **如何升级 OBD** 一节升级 OBD。如果OBD 版本低于2.9.0, 请参考文档 [OBLogProxy 安装](https://www.oceanbase.com/docs/community-oblogproxy-doc-1000000000531982) 进行部署
2. 若OBLogProxy 为 Binlog Service 模式，需要提前部署 OBProxy 和 Config Server
...
3. 部署 OBLogProxy
   ...
   ```shell
   [admin@test001 ~]$ obd cluster component add  obtest -c oblogproxy-only-example.yaml
   ```

   联网情况下，在您执行了 `obd cluster component add ` 命令之后，OBD 将检查您的目标机器是否有部署所需安装包。如果没有安装包，OBD 将自动从 YUM 源获取。

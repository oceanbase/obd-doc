...
本文以 **全组件部署（和 OceanBase 数据库一起部署）** 和 **仅部署 OBLogProxy** 两种场景为例，介绍如何使用命令行部署 OBLogProxy。
...
<main id="notice" type='explain'>
  ...
  <li>
  <p>仅部署ObLogProxy: 指当前环境中已存在 OceanBase 集群，需要使用 OBLogProxy 的服务，此时仅需要部署 OBLogProxy。如果使用 OBD 进行部署, OBD 版本需要为 2.5.0 或更高版本, 否则, 请参考文档 [OBLogProxy 安装](https://www.oceanbase.com/docs/community-oblogproxy-doc-1000000000531982) 进行部署</p>
  </li>
  <li>
  <p>若OBLogProxy 为 Binlog Service 模式，需要部署 OBProxy 和 Config Server </p>
  </li>
  </ul>
</main>
<!-- 待处理 -->

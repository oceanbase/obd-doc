...
1. 部署 OceanBase 数据库
   ...
   此处以使用配置文件 `mini-single-example.yaml` 部署 `obtest` 集群为例，您需根据实际配置文件名修改，并自定义部署集群名。联网情况下，在您执行了 `obd cluster deploy` 命令之后，obd 将检查本地镜像库中是否有部署所需安装包。如果没有安装包，obd 将自动从 YUM 源获取。

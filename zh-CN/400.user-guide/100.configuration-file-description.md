...
| enable_syslog_recycle  | 可选      | false        | 指定是否将重启前的日志文件纳入回收空间，设置为 false 时，回收日志前可容纳的日志文件数量中不包含重启前的日志。 |
...
| home_path  | 必选      | 无       | OceanBase 数据库默认安装路径。 |
...
此处以 `example/ocp-express/ocp-express-only.yaml` 文件为例介绍部署 OCP Express 时需注意的配置项。
...
此处以 `example/ob-configserver/ob-configserver-only-example.yaml` 文件为例介绍部署 Config Server 时需注意的配置项。
...
|----------|-----------|----------|----------|
| servers  | 必选      | 无        | 每台机器需要用 `- name: 机器标识名 (换行)ip: 机器 IP` 指定，多个机器就指定多次，机器标识名不能重复。</br> 在机器 IP 不重复的情况下，也可以使用 `- <ip> （换行）- <ip>` 的格式指定，此时 `- <ip>` 的格式相当于 `- name: 机器 IP（换行）ip: 机器 IP`。 |
| listen_port | 可选  | 8080  | Config Server 服务监听的端口。  |
| server_ip  | 可选  | 0.0.0.0  | 监听访问服务的 IP，只监听指定的 IP，如果有白名单需求，可以修改此项。  |
| home_path  | 必选  | 无    | 组件的工作目录。   |
| log_level | 可选   | info    | Config Server 打印的日志级别，默认为 INFO 级别。Config Server 的日志级别从低到高依次为 Debug（调试）、Info（信息）、Warn（告警）、Error（错误）、Fatal（致命）。  |
| log_maxsize | 可选   | 30    | Config Server 日志的大小，单位 MB，默认值为 30。  |
| log_maxage | 可选   | 7    | Config Server 日志保留过期文件最长时间间隔，默认值 7 天。 |
| log_maxbackups | 可选   | 10    | Config Server 最多能保留的过期日志个数。 |
| log_localtime | 可选   | true    | 控制是否使用本地时间来命名日志，默认为 true。  |
| log_compress | 可选   | true    | 控制是否开启滚动日志压缩，默认为 true。  |
| vip_address | 可选   | 无    | VIP 的 IP 地址，仅在使用 VIP 时配置。 |
| vip_port | 可选   | 无    | VIP 的端口号，仅在使用 VIP 时配置。 |
| database_type | 可选   | sqlite3    | 元数据库的数据库类型，可取值为 mysql 或 sqlite3，取值为 mysql 时可以选择使用 MySQL 数据库或 OceanBase 数据库作为 Config Server 的元数据库。  |
...
此处以 `example/grafana/grafana-only-example.yaml` 文件为例介绍部署 Grafana 时需注意的配置项。

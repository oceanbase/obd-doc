...
| ocp_meta_tenant |  必选  |  tenant_name: meta_tenant</b>max_cpu: 1</b>memory_size: 2147483648 | 重启进程  | OCP 元数据库租户信息。<ul><li>tenant_name：设置 OCP 元数据库的租户名。</li><li>max_cpu：为 OCP 元数据库分配的最大 CPU。</li><li>memory_size：为 OCP 元数据库分配的内存容量。</li></ul>  |
| ocp_meta_username | 可选 | meta | 重启进程 | 连接 OCP 元数据库租户的用户名。 |
| ocp_meta_password | 必选 | oceanbase | 重启进程  | `ocp_meta_username` 对应用户名的密码。 |
| ocp_meta_db | 可选 | meta_database | 重启进程 | OCP 元数据库 database 名。 |
| ocp_monitor_tenant | 必选 | tenant_name: monitor_tenant</b>max_cpu: 1</b>memory_size: 2147483648 | 重启进程  | OCP 监控数据库租户信息。<ul><li>tenant_name：设置 OCP 监控数据库的租户名。</li><li>max_cpu：为 OCP 监控数据库分配的最大 CPU。</li><li>memory_size：为 OCP 监控数据库分配的内存容量。</li></ul>  |
...
| ocp_monitor_db | 可选 | monitor_database | 重启进程 | OCP 监控数据库 database 名。 |
:::

...
此处以 `example/distributed-with-obproxy-example.yaml` 文件中 oceanbase-ce 组件部分为例介绍部署 OceanBase 数据库时需注意的配置项。通过配置文件部署 OceanBase 数据库流程可参见 [单机部署 OceanBase 数据库](../400.user-guide/200.start-the-oceanbase-cluster-by-using-obd.md)。
...
| obproxy_config_server_url | 可选  | 无  | Config Server 的 url，配置后可使用 ODP 代理 Config Server 中注册的所有集群。未部署 Config Server 的情况下，您也可使用 `rs_list` 配置项。  |

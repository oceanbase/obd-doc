...
| `all-components-min.yaml`/`all-components.yaml`  | 分布式部署全部组件  | OceanBase 数据库、ODP、OBAgent、OCP Express、Config Server、OBLogProxy、Prometheus、Grafana |
...
* **oblogproxy**：该目录下的配置文件示例是为部署 OBLogProxy 准备的，您可使用这些配置文件示例单独部署 OBLogProxy，或同时部署 OceanBase 数据库、ODP、Config Server 和 OBLogProxy。

...
<main id="notice" type='explain'>
  ...
  <li>
  <p>单独配置 OCP 时（使用 <code>ocp-only-example.yaml</code> 配置文件），租户信息需根据已有租户情况进行填写；创建全新的 OceanBase 数据库作为 OCP 的 MetaDB 时（使用 <code>distributed-with-obproxy-and-ocp-example.yaml</code> 配置文件），部署完成后 obd 会根据配置文件中填写的租户信息创建租户。</p>
  </li>
  </ul>
</main>

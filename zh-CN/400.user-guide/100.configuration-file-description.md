...
| local_ip | 可选      | 无        | 用于安装 OBServer 节点机器的 IP 地址，配置后，OceanBase 数据库部署会首先根据 `local_ip` 配置项进行配置，不再关注 `devname` 配置项。`local_ip` 未配置时，有如下两种情况：<ul><li>若配置了 <code>devname</code> 配置项，部署时会根据 devname 配置项进行配置</li><li>若未配置 <code>devname</code> 配置项，<code>local_ip</code> 自动赋值为 <code>servers</code> 模块配置的 IP 地址。</li></ul>详细介绍可参见 《OceanBase 数据库》中 [local_ip](https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000000220556) 一文。<blockquote>**说明**</br>该配置项自 OceanBase 数据库 V4.2.0 版本开始引入。</blockquote> |

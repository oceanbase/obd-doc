...
<main id="notice" type='explain'>
  ...
  <ul>
  <li>
  <p>Redeployment clears the data of the cluster. If you want to change the <b>Effective mode of modification</b> to <b>Redeployment</b>, you must back up the data in advance. </p>
  </li>
  <li>
  <p>If you separately deploy OCP by using the <code>ocp-only-example.yaml</code> configuration file, the tenant information is populated on the GUI based on an existing tenant. If you create an OceanBase database as the MetaDB for OCP by using the <code>distributed-with-obproxy-and-ocp-example.yaml</code> configuration file, obd creates a tenant based on the tenant information in the configuration file after OCP is deployed. </p>
  </li>
  </ul>
</main>

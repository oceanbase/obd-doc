...
This section uses the `ob-configserver-only-example.yaml` configuration file in the `example/ob-configserver` directory as an example to describe the key parameters for deploying Config Server. For more information about how to configure Config Server by using a configuration file, see [Deploy Config Server by using obd](300.command-line-operations/900.deploy-ob-configserver.md).

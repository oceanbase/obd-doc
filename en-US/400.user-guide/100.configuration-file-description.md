...
The following takes the `example/ocp-server/ocp-only-example.yaml` file as an example to describe the OCP parameters. We recommend that you deploy OCP by using the GUI of OBD. For more information, see [Deploy OCP through the GUI](200.ui-operations/300.deploy-ocp-by-ui.md).
...
| soft_dir | No | ～/ocp-server/lib | Redeployment | The directory of OCP where software packages, such as software packages of ODP and OBAgent, are stored.  |
...
| ocp_monitor_tenant | Yes | tenant_name: monitor_tenant</b>max_cpu: 1</b>memory_size: 2147483648 | Redeployment | The information about the OCP monitor tenant. <ul><li>`tenant_name`: the name of the tenant. </li><li>`max_cpu`: the maximum number of CPU cores for OCP MonitorDB. </li><li>`memory_size`: the size of memory for OCP MonitorDB. </li></ul> |

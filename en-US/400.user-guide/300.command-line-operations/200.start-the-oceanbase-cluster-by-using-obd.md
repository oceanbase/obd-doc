...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>Complete <a href="#sample-configuration-files">sample configuration files</a> for deploying a standalone OceanBase cluster are provided at the end of this topic. You can directly copy a sample configuration file and modify it. </p>
</main>
...
2. Modify the IP address, port, and related directories of each server, and specify memory-related parameters and the password of the root@sys user.
   ...
   * To deploy a standalone OceanBase cluster on multiple nodes, you can configure the same IP address for multiple servers in the `oceanbase-ce > servers` section of the configuration file. Make sure that the servers use different ports and directories. For more information, see [Sample configuration files](#sample-configuration-files).

...
You can scale out all components of an OceanBase cluster except oblogproxy and obbinlog-ce. The following example describes how to scale out OceanBase Database Community Edition of an OceanBase cluster named `test`.
...
2. Run the scale-out command. Here is an example:
   ...
   For more information about the `obd cluster scale_out` command, see [Cluster commands](../../../300.obd-command/100.cluster-command-groups.md).
...
You can add a component to a running cluster. All components that can be deployed by using obd, except `obbinlog-ce`, can be added. Note that you can add only a component that does not exist in the cluster.

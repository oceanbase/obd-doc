...
4. Configure oblogproxy.
   ...
   | servers | Yes | N/A | The servers. Specify each server in the format of `- name: server identifier(line break)ip: server IP address`. You can also specify a server in the `- <ip>` format. In this case, `- <ip>` is equivalent to `- name: server identifier(line break)ip: server IP address`. |

...
You can choose to deploy obconfigserver separately or deploy it together with OceanBase Database and OceanBase Database Proxy (ODP). This topic describes how to use obd to deploy these items together and how to use obconfigserver.
...
* obconfigserver
   ...
   obconfigserver allows you to register, store, and query the metadata of OceanBase Database. For more information, see [ob-configserver](https://github.com/oceanbase/ob-configserver/tree/main).

## Deploy obconfigserver

This section describes how to deploy obconfigserver together with OceanBase Database and ODP. After OceanBase Database is started, it is registered with obconfigserver.

<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>When deploying obconfigserver separately, you only need to delete the configuration information of OceanBase Database and ODP from the configuration file. </p>
</main>

### Step 1: Modify the configuration file

If you want to deploy OceanBase Database, ODP, and obconfigserver together, you only need to add the ob-configserver component information to the configuration file of OceanBase Database and ODP and configure the ob-configserver dependency for OceanBase Database and ODP components.
...
2. OceanBase Database
   ...
   * depends: You must configure the oceanbase-ce component to depend on the ob-configserver component. Otherwise, OceanBase Database cannot be registered with obconfigserver.

   * appname: Specifies the name of your cluster. If you do not specify this parameter, OceanBase Database cannot be registered with obconfigserver.
...
3. ODP
   ...
   For the description of the parameters in the configuration file, see [Configuration file](../100.configuration-file-description.md). Do not configure the `rs_list` parameter. Otherwise, ODP cannot obtain the cluster information registered in obconfigserver.
...
4. obconfigserver
   ...
   * `connection_url`: the URL for connecting to the database. If the database type is `sqlite3`, the default value is `$home_path/.data.db?cache=shared&_fk=1`. If the database type is `mysql`, the `connection_url` can be either an OceanBase database or a MySQL database. The specified database user must have the privileges to perform Data Definition Language (DDL) and Data Manipulation Language (DML) operations on the corresponding database in the URL. Please note that when using an OceanBase database, do not specify the database name as `oceanbase`, as it will result in an error message indicating lack of privileges.
    ...
    <main id="notice" type='explain'>
      <h4>Note</h4>
      <p>You can deploy multiple obconfigservers by configuring multiple servers under the <code>servers</code> section. In this case, OceanBase Database and ODP are by default associated with the obconfigserver that is started on the first server. </p>
    </main>
...
2. Start obconfigserver.
   ...
   As shown in the returned results, you can see the access URL of obconfigserver.
...
3. Copy the access information in the ob-configserver section.
   ...
   In the returned results, `ObRootServiceInfoUrlList` contains the information of the OceanBase cluster (obcluster) deployed together with obconfigserver, indicating that the cluster has been successfully registered with obconfigserver. You can verify whether the obconfigserver can parse the information of the OceanBase cluster by making another request to `ObRootServiceInfoUrl`. Here is the command:
...
After deploying obconfigserver, you can register the OceanBase cluster with obconfigserver by modifying the configuration file of the OceanBase cluster or executing SQL statements. The ODP started with the specified obconfigserver address can access all the OceanBase clusters registered with obconfigserver.

This section describes how to register an OceanBase cluster with obconfigserver and how to start ODP by specifying the obconfigserver address.

### Register an OceanBase cluster with obconfigserver

There are two methods to register an OceanBase cluster with obconfigserver.
...
1. Run the `obd cluster edit-config` command to edit the configuration file of an existing cluster and configure the Config URL in the cluster.
   ...
   * 10.10.10.1:8080: obconfigserver access address. This is just an example. You need to change it according to your actual situation.
...
3. Check whether the cluster is registered with obconfigserver.
...
4. Check whether obconfigserver can parse the information about the OceanBase cluster.
...
4. Exit the cluster and check whether obconfigserver can parse the information about the cluster.
...
After ODP is started through the startup option `obproxy_config_server_url`, you can use ODP to connect to the clusters registered with obconfigserver. You can run the following command to view the information about the OceanBase clusters registered with obconfigserver:

```shell
curl -s 'http://10.10.10.1:8080/services?Action=GetObProxyConfig' |jq .
```

As shown in the returned results, two clusters named obcluster and test1 are registered with obconfigserver and can be successfully connected through ODP.
...
You can run the following command to delete the information about a registered cluster from obconfigserver. The test1 cluster is used as an example here.
...
View the information about the clusters registered with obconfigserver.
...
Assume that obconfigserver uses OceanBase Database as its MetaDB. The error `prepared statement not supported` is returned when obconfigserver is started. The log is as follows:
...
* Method 1: Add `interpolateParams=true` to the `connection_url` parameter for obconfigserver in the configuration file. A sample configuration is as follows:
...
### An error is returned when ODP is used to connect to a cluster registered with obconfigserver

**Symptom**

An error is returned when ODP is used to connect to an OceanBase cluster registered with obconfigserver. Here is an example:

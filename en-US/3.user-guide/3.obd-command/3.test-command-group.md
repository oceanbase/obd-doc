...
|--- | --- | --- |--- | ---|
| --component | No | string |   | The name of the component to be tested. Valid values: `oceanbase-ce`, `oceanbase`, `obproxy-ce` and `obproxy`. If you do not specify a value, the existence of `obproxy`, `obproxy-ce`, `oceanbase`, `oceanbase-ce` is checked sequentially. The traversal stops when a component is found, and the component is then tested.   |
| --test-server | No | string | The first node of the specified component.  | Machine to be tested, followed by the `name` value corresponding to `servers` in the `yaml` file. It must be the name of a node of the specified component.  |
...
| --time | No | int | 60 | Limit for total execution time in seconds. When this option is set to 0, the running duration is not limited.  |
...
| --test-server | No | string | The name of the first node under the specified component.  | Machine to be tested, followed by the `name` value corresponding to `servers` in the `yaml` file. The name of the node to be tested under the specified component.  |

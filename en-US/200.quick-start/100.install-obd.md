...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>The <code>yum</code> command automatically installs the latest version. You can declare a version number to install the specified version. For example, if you run the <code>yum install -y ob-deploy-1.6.2</code> command to install obd, obd V1.6.2 will be installed. We recommend that you install the latest version. </p>
</main>
...
OceanBase Database provides an all-in-one package in V4.0.0 and later. You can use this package to install obd, OceanBase Database, OceanBase Database Proxy (ODP), OceanBase Agent (OBAgent), Grafana, Prometheus, and OceanBase Cloud Platform (OCP) Express V4.1.0 or later at a time.
...
1. Download the latest all-in-one package from [OceanBase Download Center](https://en.oceanbase.com/softwarecenter) and copy it to the central control server. We recommend that you download the latest version.
...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>When you install obd by using an all-in-one package, the <code>./install.sh</code> script attempts to install the package by using a user with the administrator privileges. If you forget the user password or the current user does not have the sudo privilege, you can press <code>Enter</code> three times when you are required to enter the password, and then enter <code>n</code> to use the current user for installation. The situation varies based on the user used for installation and user privileges. </p>
  <ul>
  <li>If you install obd as the <code>root</code> user or a regular user with the <code>sudo</code> privilege, the executable file of obd is stored in the <code>/usr/bin/obd</code> directory after the installation. </li>
...
<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>obd does not support source code-based installation in Python 2 in V2.0.0 and later. </p>
</main>
...
The repository information is built in obd. If the server can connect to the Internet, you do not need to configure obd. In this case, you can run the `obd mirror list` command to view the software in the obd repository.

...
Cause: You cannot modify automatic startup configurations because you do not have sudo privileges. To configure automatic startup (`enable_auto_start`) for an OBServer node, you must have sudo privileges.

Solution: Grant sudo privileges to the corresponding user account on each OBServer node. Open the `sudoers` file in the `/etc` directory as the `root` user or another user that has sudo privileges.

```shell
[root@test001 ~]# vim /etc/sudoers
```

Append the following content to the end of the `sudoers` file in the `/etc` directory. Here is an example of configuring passwordless sudo access for the `admin` user:
...
1. Install Java. For more information, see the **How do I configure the Java environment?** section in [FAQ](600.faq.md).
...
Cause: The version of the OceanBase database as the MetaDB is earlier than required. You can use only an OceanBase database of V4.2.1.0 or later as the MetaDB.

Solution: Run the `obd cluster edit-config <deploy name>` command to open the configuration file. Then, modify the version information of the `oceanbase-ce` component. The format is as follows:
...
Save the modification and run the `obd cluster redeploy <deploy name>` command to redeploy the binlog service.
...
Cause: obconfigserver is not deployed in the current business cluster. The binlog service depends on OceanBase Database Proxy (ODP) and obconfigserver. If you want to create a binlog instance in a business cluster, make sure that `obconfig_url` is configured for the cluster.

Solution: Run the `obd cluster component add <deploy_name> -c <ob-configserver.config>` command to deploy obconfigserver. For more information, see the **Add a component** section in [Scale out a cluster and change cluster components](400.user-guide/300.command-line-operations/300.manage-cluster/900.expansion-and-component-changes.md).

If ODP is deployed separately, after obconfigserver is deployed, log in to ODP as the `root@proxysys` user and run the `alter proxyconfig set obproxy_config_server_url='10.10.10.1:8080/services?Action=GetObProxyConfig';` command to modify its configurations. Then, restart ODP.

<main id="notice" type='explain'>
  <h4>Note</h4>
  <p>In this example, <code>10.10.10.1:8080</code> indicates the IP address and port number of obconfigserver. You must change them based on the actual situation.</p>
</main>

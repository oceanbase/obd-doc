|slug|test-command-group|
|---|---|

# Testing commands

OceanBase Deployer (obd) has multi-level commands. You can specify the `-h` or `--help` option in a command to query the help information of its subcommands. You can also specify the `-v` or `--verbose` option to query the execution details of a subcommand that returns an error. This topic describes how to use test commands in obd.

<main id="notice" type='notice'>
  <h4>Notice</h4>
  <p>To use the commands in this topic to test the performance of a cluster, make sure that the cluster is managed by obd. Otherwise, obd cannot obtain the cluster information. By default, a cluster deployed by using obd is managed by obd. For a cluster not deployed with obd, you first need to use obd to take over the cluster. For more information, see <a href="../400.user-guide/300.command-line-operations/400.takeover-cluster/450.takeover-cluster.md">Use obd to take over a cluster</a>. </P>
</main>
...
| --var-dir        | No    | string | ./var                   | The directory in which the system creates a log directory, which is passed to mysqltest as the value of the `--log-dir` parameter.                                                                   |
...
| --need-init | No | bool | false | Specifies whether to run the init sql files. Before obd runs the mysqltest on a new cluster, it may run some initialization files. For example, it may create some accounts or tenants required for running the test cases. This option is disabled by default.  |
...
You can use this command to run the Sysbench benchmark on a specified node of OcecanBase Database or OceanBase Database Proxy (ODP).

Before you run the Sysbench benchmark, make sure that you have installed OBClient and ob-sysbench.
...
| --script-name | No | string | point_select.lua | The name of the Sysbench script to run. You can specify multiple script names separated with commas (`,`).   |
...
| --threads | No | int | 16 | The number of initialized threads. You can specify multiple values separated with commas (`,`).  |
...
| --rand-type | No | string | special  | The random number generation function used for data access.    |
...
| -O/--optimization | No | int | 1  | The level of automatic tuning. Valid values: `0`, `1`, and `2`. The value `0` indicates to disable automatic tuning. The value `1` indicates to modify the tuning parameters that take effect without a restart. The value `2` indicates to modify all tuning parameters and restart the cluster to make the tuning take effect as needed.     |
...
| -S/--skip-cluster-status-check | No   | bool  | false      | Skip cluster status check when the option is true.   |
| -O/--optimization | No | int | 1  | The level of automatic tuning. Valid values: `0`, `1`, and `2`. The value `0` indicates to disable automatic tuning. The value `1` indicates to modify the tuning parameters that take effect without a restart. The value `2` indicates to modify all tuning parameters and restart the cluster to make the tuning take effect as needed.     |
| --parallel | No | int | max_cpu * unit_count | The degree of parallelism (DOP) for loading data. In the default value, `max_cpu` indicates the maximum number of CPU cores available for a tenant, and `unit_count` indicates the number of resource units in the resource pool used by the tenant under the test.  |
